"""Script to view scraped articles from the database."""
import sys
import pandas as pd
...
def view_articles(limit: int = None, article_id: str = None, search: str = None):
    ...
    try:
        ...
        if article_id:
            ...
            articles = [
                ...
                if search_lower in (a.title or '').lower() or 
                   ...
                   search_lower in (a.article_url or '').lower()
            ]
            if not articles:
        ...
        for idx, article in enumerate(articles, 1):
            ...
            if hasattr(article, 'article_updated') and article.article_updated:
            ...
            if len(article.content) > 500:
                print(f"\n... (truncated, {len(article.content) - 500} more characters)")
            print("-"*80)
...
def list_articles_summary():
    ...
    try:
        ...
        print(f"{'ID':<30} {'Title':<40} {'Date':<20} {'Status':<10}")
        print("-"*80)
        ...
        for article in articles:
            ...
            print(f"{article.article_id[:28]:<30} {title_short:<40} {date_str:<20} {status:<10}")
...
def show_dataframe(limit: int = None, article_id: str = None, search: str = None):
    """Display articles as a pandas DataFrame."""
    db = DatabaseManager()
    ...
    try:
        ...
        for article in articles:
            ...
            updated = article.article_updated.strftime('%Y-%m-%d %H:%M:%S') if hasattr(article, 'article_updated') and article.article_updated else None
            scraped = article.scraped_at.strftime('%Y-%m-%d %H:%M:%S') if article.scraped_at else None
            
            # Get source
            source = article.source if hasattr(article, 'source') and article.source else None
