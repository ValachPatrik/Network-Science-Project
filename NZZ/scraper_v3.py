...
class NZZScraperV3(NZZScraper):
    ...
    def __init__(self, base_url='https://www.nzz.ch', articles_url='https://www.nzz.ch/neueste-artikel?page=2000'):
        ...
        logger.info(f"Scraper v3 initialized - Articles limit: 1 year (since {self.one_year_ago.date()})")
    ...
    def _extract_author_string(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if meta_author and meta_author.get('content'):
            author_text = meta_author.get('content', '').strip()  # Only strip leading/trailing whitespace
            if author_text:
    ...
    def scrape_author_profile(self, author_url: str, author_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if self.db.author_raw_exists(author_id=author_id):
                logger.debug(f"Skipping author profile {author_id} - already exists in database")
                return None
    ...
    def _extract_departments_from_impressum(self, soup: BeautifulSoup) -> Dict[str, str]:
        ...
        department_keywords = [
            'International', 'Meinung', 'Debatte', 'Schweiz', 'Zürich', 'Wirtschaft',
            'Wissenschaft', 'Technologie', 'Mobilität', 'Feuilleton', 'Sport',
            'Wochenende', 'Gesellschaft', 'Reisen', 'Reporter', 'Nachrichten',
            'Video', 'Social Media', 'Format', 'Community', 'Podcast',
            'Audience Management', 'Visuals', 'Editorial Tech', 'Produktionsredaktion',
            'Art Director', 'Bildredaktion', 'Fotografen', 'Redaktion Deutschland',
            'Korrespondenten', 'NZZ am Sonntag', 'NZZ Folio', 'NZZ Geschichte'
        ]
        ...
        for i, heading in enumerate(headings):
            ...
            if heading_text in ['Neue Zürcher Zeitung', 'Chefredaktor', 'Stellvertreter', 'Tagesleitung', 
                               'Optimieren Sie Ihre Browsereinstellungen']:
                continue
            
            # Check if this is a department heading
            is_department = any(keyword in heading_text for keyword in department_keywords)
            ...
            if current_department:
                ...
                while current:
                    ...
                    if hasattr(current, 'find_all'):
                        ...
                        author_links = current.find_all('a', href=re.compile(r'/impressum/.*-ld\.\d+'))
        ...
        logger.info(f"Extracted {len(author_departments)} author-department mappings from impressum")
    ...
    def _impressum_author_worker(self):
            ...
            try:
                ...
                try:
                    ...
                    if not self.db.author_raw_exists(author_id=author_id):
                        ...
                        try:
                            existing = session.query(AuthorRaw).filter_by(author_id=author_id).first()
                    ...
                    failed += 1
                    logger.error(f"[{thread_short}] Error processing impressum author {name_normalized}: {str(e)}")
        ...
        logger.info(f"[{thread_short}] Impressum author worker done: {successful}✓ {failed}✗ {skipped}⊘")
    ...
    def _load_impressum_authors(self):
        ...
        try:
            ...
            logger.info(f"Queued {authors_queued} authors with profile links for parallel scraping")
            ...
            logger.info(f"STEP 1 COMPLETE: Impressum authors loaded and scraped ({total_authors} total authors, {authors_queued} profiles scraped in parallel, {saved_count} without profiles)")
            logger.info("="*80)
    ...
    def scrape_article(self, article_url: str, article_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if self.db.article_raw_exists(article_id):
                logger.debug(f"Skipping article {article_id} - already exists in database")
                return None
            ...
            tags = ', '.join(tags_list) if tags_list else None  # Convert to comma-separated string
            ...
            if article_date:
                ...
                if article_date < self.one_year_ago:
                    logger.debug(f"Skipping article {article_id} - older than 1 year ({article_date.date()})")
                    return None
            ...
            related_data = [{'id': ra['id'], 'url': ra['url']} for ra in related_articles if ra.get('id')]
            ...
            if related_articles and hasattr(self, 'scraping_queue') and hasattr(self, 'scraping_active') and self.scraping_active:
                for related_article in related_articles:
                    ...
                    if related_id and related_url:
                        ...
                        if not self.db.article_raw_exists(related_id):
                            ...
                            try:
                                self.scraping_queue.put({'id': related_id, 'url': related_url})
                                logger.debug(f"Added related article {related_id} to queue from article {article_id}")
                            except Exception as e:
                                logger.debug(f"Could not add related article {related_id} to queue: {str(e)}")
    ...
    def _article_scraping_worker(self):
        ...
        while self.scraping_active:
            try:
                ...
                try:
                    ...
                    failed += 1
                    logger.error(f"[{thread_short}] Error scraping article {article_id}: {str(e)}")
        ...
        logger.info(f"[{thread_short}] Article worker done: {successful}✓ {failed}✗ {skipped}⊘")
    ...
    def run(self, headless: bool = True, clean_authors_raw: bool = False, clean_articles_raw: bool = False):
        ...
        try:
            ...
            logger.info("Loading related articles from existing articles in database...")
                ...
                for article in all_articles_raw:
                    if article.related_articles:
                        try:
                                ...
                                if related_id and related_url:
                                    ...
                                    if not self.db.article_raw_exists(related_id):
                                        ...
                                        if related_id not in seen_ids:
                                            seen_ids.add(related_id)
                                            related_articles_to_queue.append({'id': related_id, 'url': related_url})
                            ...
                            logger.debug(f"Error parsing related articles for article {article.article_id}: {str(e)}")
                            pass
            finally:
                session.close()
            
            if articles_from_related > 0:
                logger.info(f"Found {articles_from_related} related articles to scrape (will be added to queue)")
            else:
            ...
            import threading
            logger.info("="*80)
            logger.info(f"Starting parallel article scraping threads ({self.num_worker_threads} workers)...")
            logger.info("="*80)
            ...
            logger.info(f"Started {self.num_worker_threads} article scraping threads (processing queue in real-time)")
            
            # Add related articles to queue BEFORE scrolling starts
            if articles_from_related > 0:
                logger.info(f"Adding {articles_from_related} related articles to queue...")
                ...
                logger.info(f"Added {len(related_articles_to_queue)} related articles to queue")
            ...
            logger.info("Scrolling to collect articles (1 year limit) - articles will be processed in parallel...")
            ...
            while scroll_count < max_scrolls:
                try:
                    ...
                    if scroll_count % 10 == 0:
                        ...
                        logger.info(f"  Scrolled {scroll_count} times, {articles_from_scrolling} articles queued, {queue_size} in queue...")
            ...
            logger.info(f"Scrolling complete: {articles_from_scrolling} articles added to queue")
            ...
            logger.info(f"Article scraping complete: {articles_from_scrolling} articles from scrolling, {articles_from_related} from related articles")
            logger.info("Article scraping complete, waiting for all articles to finish processing...")
    ...
    def _extract_articles_from_page_source(self, page_source: str) -> List[Dict]:
        ...
        for article_elem in article_elements:
            ...
            if time_elem:
                ...
                if datetime_attr:
                    try:
                        article_date = datetime.fromisoformat(datetime_attr.replace('Z', '+00:00'))
                    except:
                        try:
                            from dateutil import parser
