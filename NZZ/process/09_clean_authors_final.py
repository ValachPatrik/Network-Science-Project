...
def clean_authors_final():
    ...
    logger.info("  - Remove text labels from author names (Text:, Illustrationen:, etc.)")
    logger.info("="*80)
        ...
        try:
            ...
            for article in all_articles:
                try:
                    ...
                    authors_changed = (
                        len(cleaned_authors) != len(current_authors) or
                        set(cleaned_authors) != set(current_authors) or
                        authors_cleaned
                    )
                    ...
                    if authors_changed or departments_changed:
                        # Update article
                        article.authors = json.dumps(cleaned_authors) if cleaned_authors else None
                        ...
                        if items_removed:
                            logger.debug(f"Article {article.article_id}: Removed: {items_removed}")
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (cleaned: {cleaned_count}, moved to depts: {moved_to_dept_count}, removed: {removed_count}, cleaned substrings: {cleaned_substrings_count})")
            ...
            logger.info(f"Author names cleaned (substrings removed): {cleaned_substrings_count}")
