                    ...
                    try:
                        ...
                        if platform.system() == 'Windows':
                            ...
                            if os.path.exists(file_path):
                                ...
                                os.remove(file_path)
                                logger.info(f"  Successfully removed {os.path.basename(file_path)} (attempt {attempt + 1})")
                                break
                        else:
                            # Unix-like systems
                            os.remove(file_path)
                            logger.info(f"  Successfully removed {os.path.basename(file_path)}")
                            break
                    except PermissionError:
                        if attempt < 4:
                            logger.warning(f"  Permission denied, retrying in {0.5 * (attempt + 1)} seconds... (attempt {attempt + 1}/5)")
                            time.sleep(0.5 * (attempt + 1))
                        else:
                            logger.error(f"  Could not remove {os.path.basename(file_path)} after 5 attempts")
                            logger.error("  File may be locked by another process.")
                    except FileNotFoundError:
                        # File was already removed
                        break
                    except Exception as e:
                        logger.warning(f"  Error removing {os.path.basename(file_path)}: {str(e)}")
                        if attempt < 4:
                            time.sleep(0.3)
        ...
        try:
            ...
            if os.path.exists(journal_file):
                ...
                try:
                        ...
                        for attempt in range(3):
                            try:
                                ...
                                if os.path.exists(journal_file):
                                    ...
                                    logger.info(f"Journal file removed successfully (attempt {attempt + 1}).")
                                    break
                            except PermissionError:
                                if attempt < 2:
                                    logger.warning(f"Permission denied, retrying in 2 seconds... (attempt {attempt + 1}/3)")
                                    time.sleep(2.0)
                                else:
                                    logger.error("Could not remove journal file - file may be locked by another process.")
                                    logger.error("Please close all database viewers and other applications using the database.")
                            except Exception as e:
                    ...
                    if force:
                        logger.error(f"FORCE MODE: Could not remove journal file: {str(e)}")
                        logger.error("The database may be locked by another process.")
                    else:
                        logger.warning(f"Could not remove journal file: {str(e)}")
                        logger.warning("You may need to close database viewers and rerun.")
            ...
            try:
                ...
                if "database is locked" in str(e).lower():
                    if attempt < max_retries - 1:
                        logger.warning(f"  Database locked when clearing {table_name} (attempt {attempt + 1}/{max_retries}). Waiting {retry_delay} seconds...")
                        if force:
                            logger.warning("  FORCE MODE: Attempting to clean up journal files...")
                            try:
                                self._cleanup_journal_files(force=True)
                            except Exception:
                                pass
                        else:
                            logger.warning("  Please close any database viewers or other applications using the database.")
                        time.sleep(retry_delay)
                        # Increase delay slightly for subsequent attempts
                        retry_delay = min(retry_delay * 1.2, 5.0)
                        continue
                    else:
                        logger.error(f"  ERROR: Database is locked after {max_retries} attempts when clearing {table_name}")
                        if force:
                            logger.error("  FORCE MODE: Final attempt to clean up journal files...")
                            try:
                                self._cleanup_journal_files(force=True)
                                time.sleep(2.0)
                                ...
                                with self.engine.connect() as conn:
                                    ...
                                    conn.commit()
                                    deleted_count = result.rowcount
                                    logger.info(f"  Cleared {table_name}: {deleted_count} rows deleted (after force cleanup)")
                                    return deleted_count
                            except Exception:
                                pass
                        logger.error("  Please close any database viewers or other applications using the database.")
                        logger.error("  Then rerun this script.")
                        return 0
                else:
                    logger.error(f"  Database operational error when clearing {table_name}: {str(e)}")
                    return 0
            except Exception as e:
    ...
    def reset_to_preprocessing(self):
        ...
        for table in all_tables:
            if table in RAW_TABLES:
                ...
                logger.warning(f"Unknown table '{table}' - will not be cleared (not in processed tables list)")
        ...
        if not tables_to_clear:
            logger.info("No processed tables found to clear. Database is already in preprocessing state.")
            return
        ...
        logger.info("You can now run processing scripts starting from 01_process_impressum_authors.py")
        logger.info("="*80)
