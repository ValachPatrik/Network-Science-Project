...
def find_matching_author(session, author_name, author_lookup=None):
    ...
    for author in all_authors:
        if (
            normalize_name(author.name) == normalized
            or normalize_name(author.alias) == normalized
            or normalize_name(author.alt_name) == normalized
        ):
            return author
...
def get_next_non_imp_number(session):
    ...
    existing_authors = (
        session.query(Author).filter(Author.author_id.like("non-imp-%")).all()
    )
            ...
            for article in all_articles:
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Could not parse authors for article {article.article_id}"
                        )
                        continue
                    ...
                    for author_name in author_list:
                        ...
                        existing_author = find_matching_author(
                            processed_session, author_name, author_lookup
                        )
                        if existing_author:
                            matched_count += 1
                        ...
                        if created_count % 100 == 0:
                            logger.info(
                                f"Created {created_count} new authors... (matched: {matched_count}, processed: {total_author_names})"
                            )
                    ...
                    if processed_articles % 500 == 0:
                        try:
                            ...
                            logger.info(
                                f"Processed {processed_articles} articles... (created: {created_count}, matched: {matched_count})"
                            )
