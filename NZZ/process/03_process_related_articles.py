...
def process_related_articles():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Found {total_raw_articles} articles with related articles in articles_raw"
            )
            ...
            for raw_article in all_raw_articles:
                try:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Error parsing related_articles JSON for article {raw_article.article_id}: {str(e)}"
                        )
                        error_count += 1
                        continue
            ...
            logger.info(
                f"Total articles with related articles in raw: {total_raw_articles}"
            )
            logger.info("=" * 80)
