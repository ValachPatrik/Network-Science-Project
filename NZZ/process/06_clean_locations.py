...
TERMS_TO_REMOVE = {
    "text", "bilder", "illustrationen", "grafik", "interview", "infografik", 
    "fotos", "grafiken", "videos", "photos", "mitarbeit",
    "illustration", "video", "texte", "mountain view", "infografiken"
}
...
def clean_locations():
    ...
    logger.info("  - Move specific terms to departments (Bildredaktion, NZZ-Sportredaktion)")
        ...
        try:
            ...
            for article in all_articles:
                try:
                    ...
                    if article.authors:
                        try:
                            current_authors = json.loads(article.authors) if isinstance(article.authors, str) else article.authors
                        except (json.JSONDecodeError, TypeError):
                            current_authors = []
                    ...
                    locations_changed = (
                        len(cleaned_locations) != len(current_locations) or
                        set(cleaned_locations) != set(current_locations)
                    )
                    ...
                    if locations_changed or authors_changed or departments_changed:
                        # Update article
                        article.location = json.dumps(cleaned_locations) if cleaned_locations else None
                        article.authors = json.dumps(updated_authors) if updated_authors else None
                        article.department = json.dumps(updated_departments) if updated_departments else None
                        
                        cleaned_count += 1
                        
                        if items_removed:
                            logger.debug(f"Article {article.article_id}: Removed from locations: {items_removed}")
                        if items_to_departments:
                            logger.debug(f"Article {article.article_id}: Moved to departments: {items_to_departments}")
                        if items_to_authors:
                            logger.debug(f"Article {article.article_id}: Moved to authors: {items_to_authors}")
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (cleaned: {cleaned_count}, removed: {removed_count}, to depts: {moved_to_departments_count}, to authors: {moved_to_authors_count})")
