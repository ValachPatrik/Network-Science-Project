...
def extract_parts_from_parentheses(text):
    ...
    parts_without = [p.strip() for p in re.split(r'[,;]', text_without_parentheses) if p.strip()]
            ...
            for article in all_articles:
                try:
                    ...
                    for author in current_authors:
                        if isinstance(author, str) and '(' not in author and ')' not in author:
                            if author not in refined_authors:
                                refined_authors.append(author)
                    
                    for location in current_locations:
                        if isinstance(location, str) and '(' not in location and ')' not in location:
                            if location not in refined_locations:
                                refined_locations.append(location)
                    
                    for department in current_departments:
                        if isinstance(department, str) and '(' not in department and ')' not in department:
                    ...
                    for field_name, item in all_items_to_process:
                        ...
                        parts_without, parts_inside = extract_parts_from_parentheses(item)
                        ...
                        for part in all_parts:
                            ...
                            classification, normalized_value = classify_part(part, author_normalizer)
                            ...
                            if classification == 'author' and normalized_value:
                                ...
                                if parsed and len(parsed) > 0 and parsed[0].normalized_name:
                                    ...
                                    if parsed[0].normalized_name not in refined_authors:
                                        refined_authors.append(parsed[0].normalized_name)
                                else:
                    ...
                    article.authors = json.dumps(refined_authors) if refined_authors else None
                    article.location = json.dumps(refined_locations) if refined_locations else None
                    article.department = json.dumps(refined_departments) if refined_departments else None
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (refined: {refined_count})")
