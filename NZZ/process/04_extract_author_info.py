...
def extract_author_info():
    ...
    logger.info("="*80)
    logger.info("Extracting author info from author strings using geopy (no LLM for speed)")
    logger.info("="*80)
    ...
    try:
        ...
        try:
            ...
            all_raw_articles = raw_session.query(ArticleRaw).filter(
                ArticleRaw.author.isnot(None),
                ArticleRaw.author != ''
            ).all()
            
            total_raw_articles = len(all_raw_articles)
            logger.info(f"Found {total_raw_articles} articles with author information in articles_raw")
            ...
            for raw_article in all_raw_articles:
                try:
                    ...
                    if processed_article.authors and processed_article.location and processed_article.department:
                        skipped_count += 1
                        logger.debug(f"Article {raw_article.article_id} already has all author info, skipping")
                        continue
                    ...
                    parsed_authors = author_normalizer.parse_author_string(author_string)
                    ...
                    for parsed_author in parsed_authors:
                        ...
                        if parsed_author.location:
                            ...
                            if '(' in location or ')' in location or '[' in location or ']' in location:
                                logger.debug(f"Article {raw_article.article_id}: Skipping location '{location}' (contains brackets/parentheses)")
                                continue
                            
                            # Skip if it's a department (double-check with normalizer)
                            if author_normalizer.is_department(location, context=author_string):
                                logger.debug(f"Article {raw_article.article_id}: Skipping location '{location}' (identified as department)")
                                continue
                            ...
                            if len(location_words) >= 2:
                                ...
                                both_capitalized = all(w[0].isupper() if w else False for w in location_words[:2])
                                has_location_keyword = any(kw in location.lower() for kw in ['valley', 'city', 'town', 'gazastreifen', 'gaza', 'de ', ' am ', ' on ', ' in ', ' bei ', ' an ', ' al-', ' al '])
                                ...
                                if both_capitalized and not has_location_keyword:
                                    ...
                                    if not author_normalizer.is_location(location, context=author_string):
                                        logger.debug(f"Article {raw_article.article_id}: Skipping location '{location}' (looks like a name, not confirmed as location)")
                                        continue
                    ...
                    if article_author_names:
                        ...
                        logger.debug(f"Article {raw_article.article_id}: Extracted authors {article_author_names} from '{author_string}'")
                    ...
                    if article_locations:
                        ...
                        logger.debug(f"Article {raw_article.article_id}: Extracted locations {article_locations} from '{author_string}'")
                    
                    if article_departments:
                        if not processed_article.department:
                            processed_article.department = json.dumps(article_departments)
                            articles_with_departments += 1
                        logger.debug(f"Article {raw_article.article_id}: Extracted departments {article_departments} from '{author_string}'")
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (authors: {articles_with_authors}, locations: {articles_with_locations}, departments: {articles_with_departments})")
            ...
            logger.info(f"Articles with departments extracted: {articles_with_departments}")
            logger.info(f"Articles skipped (not in processed table or already has info): {skipped_count}")
