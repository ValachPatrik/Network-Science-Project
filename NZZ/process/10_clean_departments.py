                ...
                try:
                    ...
                    for dept in current_departments:
                        ...
                        if ',' in cleaned_dept:
                            ...
                            parts = [p.strip() for p in cleaned_dept.split(',') if p.strip()]
                            for part in parts:
                                part_cleaned = clean_department_name(part)
                                if part_cleaned and part_cleaned not in cleaned_departments:
                    ...
                    departments_changed = (
                        has_changes or 
                        set(cleaned_departments) != set(current_departments)
                    )
                    
                    if departments_changed or authors_changed:
                        # Update article
                        article.department = json.dumps(cleaned_departments) if cleaned_departments else None
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (cleaned: {cleaned_count}, depts cleaned: {departments_cleaned}, moved to authors: {moved_to_authors_count})")
