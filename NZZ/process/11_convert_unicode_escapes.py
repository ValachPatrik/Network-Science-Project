...
def convert_unicode_escapes():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Found {total_articles} articles with authors/locations/departments")
            ...
            for article in all_articles:
                try:
                    ...
                    if article.authors:
                        try:
                            processed_authors, needs_update = process_json_field(article.authors)
                            if needs_update and isinstance(processed_authors, list):
                                ...
                                new_json = json.dumps(processed_authors, ensure_ascii=False)
                                ...
                                if new_json != article.authors:
                                    ...
                                    if converted_count < 3:
                                        ...
                                        logger.info(f"Example conversion {converted_count + 1}: Article {article.article_id}")
                                    ...
                                    for author in processed_authors:
                                        if isinstance(author, str) and any(ord(c) > 127 for c in author):
                                            authors_converted += 1
                        except Exception as e:
                            logger.warning(f"Error processing authors for article {article.article_id}: {str(e)}")
                    
                    # Process locations
                    if article.location:
                        try:
                            processed_locations, needs_update = process_json_field(article.location)
                            if needs_update and isinstance(processed_locations, list):
                                new_json = json.dumps(processed_locations, ensure_ascii=False)
                                if new_json != article.location:
                                    article.location = new_json
                                    has_changes = True
                                    for location in processed_locations:
                                        if isinstance(location, str) and any(ord(c) > 127 for c in location):
                                            locations_converted += 1
                        except Exception as e:
                            logger.warning(f"Error processing locations for article {article.article_id}: {str(e)}")
                    
                    # Process departments
                    if article.department:
                        try:
                            processed_departments, needs_update = process_json_field(article.department)
                            if needs_update and isinstance(processed_departments, list):
                                new_json = json.dumps(processed_departments, ensure_ascii=False)
                                if new_json != article.department:
                                    article.department = new_json
                                    has_changes = True
                                    for dept in processed_departments:
                                        if isinstance(dept, str) and any(ord(c) > 127 for c in dept):
                                            departments_converted += 1
                        except Exception as e:
                            logger.warning(f"Error processing departments for article {article.article_id}: {str(e)}")
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (converted: {converted_count}, authors: {authors_converted}, locations: {locations_converted}, depts: {departments_converted})")
