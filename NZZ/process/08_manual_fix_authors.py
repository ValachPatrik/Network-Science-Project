...
def manual_fix_authors():
    ...
    try:
        ...
        try:
            ...
            suspicious_items = []
            suspicious_author_to_articles = (
                {}
            )  # Map author -> list of (article, all_authors, author_index)
            ...
            logger.info(
                f"Found {total_suspicious} unique suspicious author names ({total_instances} total instances)"
            )
            ...
            logger.info(
                "  - Enter comma-separated names for authors (e.g., 'Name1, Name2')"
            )
            logger.info(
                "  - Use 'dept:Name' to move parts to departments (e.g., 'dept:Bildredaktion NZZ')"
            )
            logger.info(
                "  - Use 'loc:Name' to move parts to locations (e.g., 'loc:Berlin')"
            )
            ...
            for idx, item in enumerate(suspicious_items, 1):
                ...
                print("\n" + "=" * 80)
                print(
                    f"Item {idx}/{total_suspicious} (appears in {instance_count} article(s))"
                )
                print("=" * 80)
                ...
                print(
                    f"Example - All authors in first article: {json.dumps(first_instance['all_authors'], ensure_ascii=False)}"
                )
                print("=" * 80)
                ...
                while True:
                    try:
                        ...
                        print("\nQuick shortcuts:")
                        print(
                            "  - Type comma-separated names to replace (e.g., 'Name1, Name2')"
                        )
                        print(
                            "  - Type '2' to automatically split every 2 words with commas"
                        )
                        print("  - Use 'dept:Name' or 'loc:Name' to move parts")
                        print(
                            "  - Use pattern 's/old/new/' to replace (e.g., 's/ /, /' adds commas)"
                        )
                        ...
                        if use_readline:
                            try:
                                readline.set_startup_hook(
                                    lambda: readline.insert_text(author)
                                )
                                prompt = "Your input: "
                            except Exception:
                                use_readline = False
                        ...
                        if user_input_lower == "2":
                            ...
                            if len(words) >= 2:
                                ...
                                print(
                                    "Not enough words to split (need at least 2 words)."
                                )
                                continue
                        ...
                        # Check for pattern replacement (s/old/new/)
                            ...
                            if len(parts) >= 3:
                                ...
                                if modified_author != author:
                                    ...
                                    print(
                                        f"Applied pattern: '{old_text}' -> '{new_text}'"
                                    )
                                    print(f"Result: {modified_author}")
                                else:
                                    print(
                                        f"Pattern '{old_text}' not found in current value."
                                    )
                                    continue
                        ...
                        if user_input_lower in ["quit", "q"]:
                            ...
                            logger.info(
                                f"  Moved to departments: {moved_to_dept_count}"
                            )
                        ...
                        if user_input_lower in ["delete", "d"]:
                            ...
                            for inst in instances:
                                ...
                                updated_authors = [
                                    a for a in all_authors if a != author
                                ]
                        ...
                        if user_input_lower == "dept":
                            ...
                            for inst in instances:
                                ...
                                article.department = format_author_list(
                                    current_departments
                                )
                            ...
                            logger.info(
                                f"Moved to departments in {instance_count} article(s)"
                            )
                            fixed_count += instance_count
                            break
                        ...
                        for inst in instances:
                            ...
                            # Update authors
                            updated_authors = (
                                all_authors[:author_index]
                                + new_authors
                                + all_authors[author_index + 1 :]
                            )
                            article.authors = format_author_list(updated_authors)
                            ...
                            if new_departments:
                                ...
                                for dept in new_departments:
                                    ...
                                    dept_name = TERMS_TO_DEPARTMENTS.get(
                                        dept.lower(), dept
                                    )
                                    if dept_name not in current_departments:
                                        current_departments.append(dept_name)
                        ...
                        fixed_count += instance_count
                        moved_to_dept_count += (
                            len(new_departments) * instance_count
                            if new_departments
                            else 0
                        )
                        moved_to_loc_count += (
                            len(new_locations) * instance_count if new_locations else 0
                        )

                        logger.info(f"Fixed in {instance_count} article(s).")
                        if new_authors:
                            logger.info(
                                f"  Authors: {json.dumps(new_authors, ensure_ascii=False)}"
                            )
                        if new_departments:
                            logger.info(
                                f"  Departments: {json.dumps(new_departments, ensure_ascii=False)}"
                            )
                        if new_locations:
                            logger.info(
                                f"  Locations: {json.dumps(new_locations, ensure_ascii=False)}"
                            )
                        break
                ...
                if idx % 10 == 0:
                    try:
                        processed_session.commit()
                        logger.info(
                            f"Progress saved (processed {idx}/{total_suspicious} unique authors)"
                        )
