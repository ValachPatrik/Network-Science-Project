            ...
            with self.engine.connect() as conn:
                ...
                if table_exists:
                    ...
                    if not required_columns.issubset(existing_columns):
                        logger.info(
                            "Articles table has old schema. Dropping and recreating with new schema..."
                        )
                        conn.execute(text("DROP TABLE IF EXISTS articles"))
                        conn.commit()
                        logger.info(
                            "Old articles table dropped. Will be recreated with correct schema."
                        )
                    else:
...
def process_articles():
    ...
    try:
        ...
        try:
            ...
            for raw_article in all_raw_articles:
                try:
                    ...
                    if existing_article:
                        ...
                        existing_article.article_date_updated = (
                            raw_article.article_updated
                        )
                    ...
                    if (processed_count + updated_count) % 100 == 0:
                        ...
                        logger.info(
                            f"Processed {processed_count + updated_count} articles..."
                        )
            ...
            logger.info("=" * 80)
            logger.info(
                "Note: authors, department, location, and related_articles columns"
            )
            logger.info(
                "      are left empty and will be populated by later processing steps."
            )
            logger.info("=" * 80)
