            ...
            with self.engine.connect() as conn:
                ...
                if table_exists:
                    ...
                    if (
                        ...
                        or "alternate_name" in existing_columns
                    ):
                        logger.info(
                            "Authors table has old schema. Dropping and recreating with new schema..."
                        )
                        conn.execute(text("DROP TABLE IF EXISTS authors"))
                        conn.commit()
                        logger.info(
                            "Old authors table dropped. Will be recreated with correct schema."
                        )
                    else:
            ...
            for raw_author in all_raw_authors:
                try:
                    ...
                    if raw_author.author_id:
                        existing_author = (
                            processed_session.query(Author)
                            .filter_by(author_id=raw_author.author_id)
                            .first()
                        )
                    else:
                        ...
                        existing_author = (
                            processed_session.query(Author)
                            .filter_by(name=raw_author.name, author_id=None)
                            .first()
                        )
                    ...
                    if existing_author:
                        ...
                        existing_author.title = (
                            raw_author.title or existing_author.title
                        )
                        existing_author.alt_name = (
                            raw_author.alt_name or existing_author.alt_name
                        )
                        existing_author.alias = (
                            raw_author.alias or existing_author.alias
                        )
                        existing_author.department = (
                            raw_author.department or existing_author.department
                        )
                    ...
                    if (processed_count + updated_count) % 100 == 0:
                        ...
                        logger.info(
                            f"Processed {processed_count + updated_count} authors..."
                        )

                except Exception as e:
                    error_count += 1
                    logger.error(
                        f"Error processing author {raw_author.id} ({raw_author.name}): {str(e)}"
                    )
