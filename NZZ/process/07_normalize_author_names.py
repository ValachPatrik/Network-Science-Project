...
def split_authors_string(author_string, author_normalizer):
    ...
    if len(words) >= 6:
        ...
        split_authors = split_by_name_pattern_conservative(author_string, author_normalizer)
        ...
        try:
            ...
            for article in all_articles:
                try:
                    ...
                    for author in current_authors:
                        ...
                        if ',' in author:
                            ...
                            for part in parts:
                                parsed = author_normalizer.parse_author_string(part)
                                if parsed and len(parsed) > 0 and parsed[0].normalized_name:
                                    if parsed[0].normalized_name not in normalized_authors:
                                        normalized_authors.append(parsed[0].normalized_name)
                                else:
                                    if part not in normalized_authors:
                                        normalized_authors.append(part)
                        else:
                            # No commas - try to split
                            split_authors = split_authors_string(author, author_normalizer)
                            for split_author in split_authors:
                                if split_author and split_author not in normalized_authors:
                    ...
                    authors_changed = (
                        len(normalized_authors) != len(current_authors) or
                        set(normalized_authors) != set(current_authors)
                    )
                    departments_changed = len(items_to_departments) > 0
                    
                    if authors_changed or departments_changed:
                        # Update article
                        article.authors = json.dumps(normalized_authors) if normalized_authors else None
                        ...
                        if authors_changed:
                            logger.debug(f"Article {article.article_id}: Normalized authors: {current_authors} -> {normalized_authors}")
                    ...
                    if updated_count % 100 == 0:
                        try:
                            ...
                            logger.info(f"Processed {updated_count} articles... (normalized: {normalized_count}, to depts: {moved_to_departments_count})")
