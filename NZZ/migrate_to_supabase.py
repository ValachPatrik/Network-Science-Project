...
try:
    ...
    HAS_PSYCOPG2 = False
    print("Error: psycopg2 is required. Install with: pip install psycopg2-binary python-dotenv")
        ...
        if col['is_pk'] and col['type'].upper() == 'INTEGER':
            ...
            if col['default'] is not None and col['default'] != '':
                ...
                if isinstance(default_val, str):
                    ...
                    if default_val.upper().startswith('CURRENT_TIMESTAMP') or 'NOW()' in default_val.upper():
...
def migrate_table_data(sqlite_conn, pg_conn, table_name, primary_key=None, batch_size=1000):
    ...
    while True:
        ...
        try:
            ...
            if rows_migrated % (batch_size * 10) == 0:
                logger.info(f"Migrated {rows_migrated:,} / {total_rows:,} rows ({rows_migrated/total_rows*100:.1f}%)")
...
def migrate_database():
    ...
    if not HAS_PSYCOPG2:
        logger.error("psycopg2 is required. Install with: pip install psycopg2-binary python-dotenv")
        return
    ...
    try:
        ...
        for table_name in tables:
            ...
            try:
                ...
                sqlite_cursor.execute(f"SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='{table_name}' AND sql IS NOT NULL")
                indexes = []
                for idx_name, idx_sql in sqlite_cursor.fetchall():
                    if idx_name.startswith('sqlite_') or 'AUTOINCREMENT' in idx_sql.upper():
                    ...
                    if 'ON' in idx_sql.upper():
                        ...
                        if len(parts) > 1:
                            ...
                            raw_cols = [c.strip().strip('"').strip("'") for c in col_part.split(',')]
                            ...
                            for raw_col in raw_cols:
                                ...
                                for schema_col in schema:
                                    if schema_col['name'].upper() == raw_col.upper():
                                        matched = schema_col['name']  # Use actual case from schema
                                        break
                                if matched:
                                    cols.append(matched)
                                else:
