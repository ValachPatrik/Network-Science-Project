...
def main():
    ...
    if (
        not scraper.author_normalizer.llm_classifier
        or not scraper.author_normalizer.llm_classifier.use_llm
    ):
    ...
    print(
        "Note: This will use infinite scroll (like the scraper) and stop after 50 articles"
    )
    print("=" * 80)
    print()
    ...
    def limited_get_article_list():
        ...
        try:
            ...
            while len(all_articles) < max_articles:
                ...
                current_articles = scraper._extract_articles_from_page_source(
                    page_source
                )
                ...
                if scroll_count % 5 == 0:
                    print(
                        f"  Scrolled {scroll_count} times, found {len(all_articles)} articles so far..."
                    )
                ...
                if scroll_count > 100:
                    print(
                        f"  Reached safety limit of 100 scrolls, stopping with {len(all_articles)} articles"
                    )
                    break
    ...
    print("=" * 80)
    print(
        "Step 2: Processing articles and extracting authors (this may take a few minutes)..."
    )
    print("=" * 80)
    print()
    ...
    for i, article in enumerate(articles, 1):
        ...
        print(
            f"[{i:2d}/{len(articles)}] Article {article_id}: {article_title[:50]}...",
            end=" ... ",
            flush=True,
        )
        ...
        try:
            ...
            for author in authors:
                ...
                parsed_authors = scraper.author_normalizer.parse_author_string(
                    author_name
                )
                ...
                if parsed_authors:
                    ...
                    if (
                        len(author_name.split()) == 1
                        and author_name[0].isupper()
                        and len(author_name) >= 3
                    ):
            ...
            location_str = (
                f" [Loc: {', '.join(article_locations)}]" if article_locations else ""
            )
            dept_str = (
                f" [Dept: {', '.join(result_entry['departments'])}]"
                if result_entry["departments"]
                else ""
            )
            print(
                f"OK: {len(authors)} extracted, {len(parsed_authors_list)} parsed: {names_str[:40]}{location_str}{dept_str}"
            )
    ...
    json_output = {
        "test_metadata": {
            ...
            "success_rate": (
                f"{(total_parsed_authors / total_author_strings * 100):.1f}%"
                if total_author_strings > 0
                else "0%"
            ),
        ...
        "statistics": {
            ...
            "unique_departments": (
                sorted(set(all_departments)) if all_departments else []
            ),
    ...
    # Save JSON to file
    json_filename = (
        f"author_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
