...
log_file = os.path.join(
    log_dir, f'nzz_scraper_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
)
error_log_file = os.path.join(
    log_dir, f'nzz_scraper_errors_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
)
...
def analyze_article(
    ...
    for idx, author in enumerate(authors):
        ...
        step_analysis["steps"].append(
            {
                ...
                "result": (
                    len(author_name.split()) == 1 and author_name[0].isupper()
                    if author_name
                    else False
                ),
            }
        ...
        step_analysis["steps"].append(
            {
                ...
                "result": (
                    [p.__dict__ for p in parsed_authors] if parsed_authors else []
                ),
            }
        )
        ...
        if parsed_authors:
            ...
            if scraper.author_normalizer.is_location(author_name, context=author_name):
                ...
                if len(words) >= 2:
                    ...
                    has_location_keyword = any(
                        kw in author_name.lower() for kw in location_keywords
                    )
                    has_location_connector = any(
                        conn in author_name.lower()
                        for conn in [
                            " de ",
                            " am ",
                            " on ",
                    ...
                    if has_location_keyword or (
                        has_location_connector and is_capitalized
                    ):
                        is_standalone_location = True
            ...
            if is_standalone_location:
                ...
                step_analysis["issues"].append(
                    "Parsing returned empty result - fallback used"
                )
        ...
        processing_detail = {"extracted_string": author_name, "parsed_results": []}
                ...
                processing_detail["parsed_results"].append(
                    {
                        "normalized_name": author_name,
                        "first_name": (
                            author_name.split()[0] if author_name.split() else ""
                        ),
                        "last_name": (
                            " ".join(author_name.split()[1:])
                            if len(author_name.split()) > 1
...
def main():
    ...
    try:
        ...
        while len(all_articles) < 1000:
            ...
            if scroll_count > 500:
                print(
                    f"  Reached safety limit of 500 scrolls, stopping with {len(all_articles)} articles"
                )
                break
    ...
    for idx, article in enumerate(articles, 1):
        ...
        try:
            ...
            analysis = analyze_article(
                article_id, article_title, article_url, authors, scraper
            )
            ...
            if analysis["issues"]:
                ...
                print(
                    f"  -> OK: {len(authors)} extracted, {analysis['num_parsed']} parsed"
                )

            # Print detailed info for first 10 articles
            if idx <= 10:
                print(
                    f"    Parsed {len(analysis['parsed_authors'])} normalized author(s):"
                )
    ...
    output_file = (
        f"author_test_results_1000_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
