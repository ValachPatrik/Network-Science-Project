...
def export_data_to_json():
    ...
    try:
        ...
        articles = session.query(Article).order_by(Article.scraped_at.desc()).limit(10).all()
        articles_data = []
        for article in articles:
            article_dict = {
                ...
                'title': article.title,
                'content': article.content[:500] + '...' if article.content and len(article.content) > 500 else article.content,  # Truncate for readability
                'content_length': len(article.content) if article.content else 0,
                'tags': article.tags,
                'category': article.category,
                'article_url': article.article_url,
                'article_date': article.article_date.isoformat() if article.article_date else None,
                'article_updated': article.article_updated.isoformat() if article.article_updated else None,
        ...
        print(f"[OK] Exported {len(related_articles_data)} related articles to {related_file}")
        ...
        for author in authors:
            author_dict = {
                ...
                'scraped_at': author.scraped_at.isoformat() if author.scraped_at else None
            }
        ...
        associations_file = os.path.join(test_data_dir, 'article_author_associations.json')
        with open(associations_file, 'w', encoding='utf-8') as f:
            json.dump(associations_data, f, indent=2, ensure_ascii=False)
        print(f"[OK] Exported {len(associations_data)} article-author associations to {associations_file}")
    ...
    try:
        ...
        if article_data:
            ...
            for related in related_articles[:5]:  # Show first 5
                print(f"    - {related.related_article_id}: {related.related_article_url[:60]}...")
...
def test_database_tables():
    ...
    try:
        ...
        required_tables = ['articles', 'related_articles', 'authors', 'article_author_association']
...
def test_impressum_loading():
    ...
    print(f"[PASS] Loaded {len(scraper.currently_employed_authors)} currently employed authors from impressum")
    ...
    print(f"  Found {len(found_authors)}/{len(known_authors)} known authors in impressum")
...
def test_author_without_link():
    ...
    if db.author_exists(name=test_name):
        ...
        session = db.Session()
        existing = session.query(Author).filter_by(name=test_name).filter(Author.author_id.like('no-id-%')).first()
        if not existing:
    ...
    try:
        ...
        fresh_author = session.query(Author).filter_by(name=test_name).filter(Author.author_id.like('no-id-%')).first()
        if not fresh_author:
    ...
    if not author_id_val or not author_id_val.startswith('no-id-'):
        print(f"[FAIL] Author ID should be a placeholder 'no-id-...' for authors without links, got: {author_id_val}")
        return False
    
    if not author_url_val or not author_url_val.startswith('no-url-'):
        print(f"[FAIL] Author URL should be a placeholder 'no-url-...' for authors without links, got: {author_url_val}")
        return False
...
def test_author_with_city_in_name():
    ...
    for author in authors_without_links[:5]:  # Show first 5
        print(f"  - {author.name} (employed: {author.currently_employed}, alias: {author.alias})")
...
def test_author_exists_by_name_or_alias():
    ...
    print(f"[PASS] Created test author: {author_name} (alias: {author_alias}, id: {author_id_val})")
...
def test_scrape_multiple_articles():
    ...
    test_articles = [
        ("https://www.nzz.ch/feuilleton/eric-kaufmann-wokeness-hamas-universitaeten-ld.1764342", "1764342"),
        ("https://www.nzz.ch/international/der-anschlag-in-manchester-ist-ausdruck-eines-weit-verbreiteten-judenhasses-ld.1906060", "1906060"),
        ("https://www.nzz.ch/feuilleton/was-zum-teufel-ist-aus-grossbritannien-geworden-die-verhaftung-eines-komikers-loest-eine-debatte-aus-ld.1900909", "1900909"),
        ("https://www.nzz.ch/feuilleton/schweizer-hochschulen-antisemitische-tweets-ld.1765585", "1765585"),
        ("https://www.nzz.ch/feuilleton/shai-davidai-columbia-harvard-hamas-terror-ld.1762332", "1762332"),
        ("https://www.nzz.ch/feuilleton/studentische-hamas-fans-die-den-progrom-feiern-ld.1761502", "1761502"),
        ("https://www.nzz.ch/international/antisemitismus-manchester-ld.1905290", "1905290"),
        ("https://www.nzz.ch/international/judenhass-europa-ld.1905501", "1905501"),
        ("https://www.nzz.ch/international/israel-palastina-konflikt-ld.1850912", "1850912"),
        ("https://www.nzz.ch/feuilleton/grossbritannien-komiker-verhaftung-ld.1898033", "1898033"),
        ("https://www.nzz.ch/feuilleton/frankreich-karikaturist-plantu-ld.1844429", "1844429"),
        ("https://www.nzz.ch/feuilleton/ukraine-selenski-ld.1863736", "1863736"),
        # Add more articles to ensure we have enough data
        ("https://www.nzz.ch/international/krieg-in-der-ukraine-raketenalarm-in-russland-ld.1900000", "1900000"),  # May not exist, will skip
        ("https://www.nzz.ch/wirtschaft/migros-renditen-marktniveau-ld.1900001", "1900001"),  # May not exist, will skip
    ]
    ...
    for idx, (url, article_id) in enumerate(test_articles, 1):
        try:
            ...
            if db.article_exists(article_id):
                print(f"  [{idx}/{len(test_articles)}] Article {article_id} already exists, skipping...")
                skipped_count += 1
                continue
    ...
    for related_id in list(unique_related_ids)[:20]:
        ...
        related = db.Session().query(RelatedArticle).filter_by(related_article_id=related_id).first()
        if not related:
            continue
        
        try:
            print(f"  Scraping related article: {related_id}...")
            article_data = scraper.scrape_article(related.related_article_url, related_id)
            ...
            if article_data and article_data.get('content'):
                ...
                print(f"    [OK] Saved related article: {article_data.get('title', 'N/A')[:50]}...")
        except Exception as e:
    ...
    while iteration < max_iterations:
        ...
        from sqlalchemy import text
        total_associations = db.Session().execute(text("SELECT COUNT(*) FROM article_author_association")).scalar()
        ...
        for article in articles[iteration*5:(iteration+1)*5]:
            try:
                ...
                for author in authors:
                    if 'id' in author and author.get('id'):
                        ...
                        if not db.author_exists(author_id):
                            print(f"  Scraping author profile: {author['name']} ({author_id})...")
                            author_data = scraper.scrape_author_profile(author_url, author_id)
                ...
                print(f"    [WARN] Error processing article {article.article_id}: {str(e)}")
                continue
    ...
    if total_articles >= 10 and total_related >= 10 and total_authors >= 10 and total_associations >= 10:
