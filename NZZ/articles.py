...
class GraphVisualizer:
    ...
    def _filter_and_prepare_graph(self, G, node_scale, weight_threshold):
        """
        Helper method to filter edges and compute initial node sizes.
        Returns the filtered graph H, node_weights, and normalized node_sizes.
        """
    ...
    def _calculate_layout(self, H, cluster_colors, iterations):
        """
        Helper method to compute the graph layout using ForceAtlas2.
        Adapts the calculation if cluster information is provided.
        """
        # Configure the base ForceAtlas2 object
        ...
        if cluster_colors is not None:
            ...
            # --- Initial position generation based on clusters ---
            cluster_groups = {}
            for node in H.nodes():
    ...
    def _draw_graph(self, H, pos, node_sizes, cluster_colors, figsize):
        """
        Helper method to draw the graph nodes and edges using NetworkX and Matplotlib.
        """
        fig, ax = plt.subplots(figsize=figsize)

        # 1. Determine Node Colors
        if cluster_colors is not None:
    ...
    def _setup_interactivity(
        ...
        """
        Helper method to set up scroll zoom, pan, and dynamic label updates.
        """
        ...
        def update_labels():
            ...
            for node, _ in top_nodes:
                if show_names:
                    ...
                    # If it's still a technical ID, try a cleaner fallback
        ...
        # --- Add scroll zoom and pan event handlers ---
        def zoom(event):
            base_scale = 1.1
    ...
    def visualize_existing_graph_interactive(
        ...
        """
        Public entry point to visualize a graph interactively.
        Orchestrates filtering, layout computation, drawing, and interactivity setup.
        """
        ...
        plt.show(block=True)
        return H  # Optionally return the filtered graph
