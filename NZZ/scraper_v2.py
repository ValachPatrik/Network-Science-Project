...
class NZZScraperV2(NZZScraper):
    ...
    def __init__(self, base_url='https://www.nzz.ch', articles_url='https://www.nzz.ch/neueste-artikel?page=2000'):
        ...
        self.related_articles_queue = Queue()  # Queue for related articles to be scraped
        ...
        self.author_normalizer = AuthorNormalizer(use_geopy=True, use_llm=True, llm_model_name=llm_model_name)
        self.currently_employed_authors = set()  # Cache of currently employed author names
    ...
    def _extract_related_articles(self, soup: BeautifulSoup, article_id: str) -> List[Dict]:
        ...
        related_containers = soup.find_all(['div', 'section'], {
            'data-team-more-to-subject': True
        })
        
        # Also look for divs with class containing "max-w-[640px]" which often contain related articles
        related_containers.extend(soup.find_all('div', {
            'class': lambda x: x and 'max-w-[640px]' in str(x)
        }))
        ...
        for container in related_containers:
            ...
            for article_elem in article_elements:
                ...
                title = None
                title_elem = article_elem.find('span', class_=lambda x: x and 'chartbeat-teaser-title' in str(x))
                if title_elem:
                ...
                if time_elem:
                    ...
                    if datetime_attr:
                        try:
                            ...
                            related_date = datetime.fromisoformat(datetime_attr.replace('Z', '+00:00'))
                        except:
                            try:
    ...
    def _load_impressum_authors(self):
        ...
        try:
            ...
            author_pattern = re.compile(r'([A-ZÄÖÜ][a-zäöüß]+(?:\s+[A-ZÄÖÜ][a-zäöüß]+)*)\s*\(([a-zäöüß]{1,4}\.)\)', re.IGNORECASE)
            ...
            for name_normalized, profile_info in author_profiles.items():
                try:
                    ...
                    if not self.db.author_exists(author_id=author_id):
                        ...
                        try:
                            existing_author = session.query(Author).filter_by(author_id=author_id).first()
                            if existing_author:
                    ...
                    logger.warning(f"Error processing impressum author profile {name_normalized}: {str(e)}")
            ...
            for name, abbrev in matches:
                ...
                if not self.db.author_exists(name=name_normalized):
                    try:
                        ...
                        logger.debug(f"Error saving impressum author {name_normalized}: {str(e)}")
                else:
                    # Update existing author to mark as currently employed
                    existing_author = self.db.get_author_by_name_or_alias(name=name_normalized)
                    if existing_author:
                        # Update alias to include name if not already there
                        alias_list = []
                        if existing_author.alias:
                            alias_list = [a.strip() for a in existing_author.alias.split(',')]
                        if name_normalized not in alias_list:
                            alias_list.append(name_normalized)
                        alias_str = ', '.join(alias_list)
                        ...
                        session = self.db.Session()
                        try:
                            fresh_author = session.query(Author).filter_by(name=name_normalized).first()
                            if fresh_author:
            ...
            logger.info(f"Loaded {len(self.currently_employed_authors)} currently employed authors from impressum, scraped {scraped_count} author profiles, saved {saved_count} authors to database")
    ...
    def _is_author_currently_employed(self, author_name: str) -> bool:
        ...
        # Check exact match
        if author_name in self.currently_employed_authors or base_name in self.currently_employed_authors:
            return True
        ...
        for employed_name in self.currently_employed_authors:
            ...
            if employed_base in author_name or author_name in employed_base or base_name in employed_base:
                return True
    ...
    def _extract_authors_from_article(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        author_divs = soup.find_all('div', {
            'class': lambda x: x and 'mr-2' in str(x) and 'text-sm' in str(x)
        })
        ...
        for div in author_divs:
            ...
            for span in spans:
                ...
                for author_name in individual_authors:
                    ...
                    base_text = text_parts[0].strip().lower() if text_parts else author_name.lower()
                    ...
                    if base_text in [',', '.', 'und', 'and', 'von', 'von', 'der', 'die', 'das']:
                        continue
            ...
            if author_text:
                ...
                if author_text:
                    ...
                    for author_name in individual_authors:
                        ...
                        base_name = name_parts[0].strip().lower() if name_parts else author_name.lower()
    ...
    def scrape_author_profile(self, author_url: str, author_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            bio = None
            bio_elem = soup.find('p', class_=lambda x: x and 'articlecomponent' in str(x))
            if bio_elem:
            ...
            if img:
                img_src = img.get('src', '')
                if img_src:
                    image_url = self._make_absolute_url(img_src) if not img_src.startswith('http') else img_src
            ...
            if not name:
                logger.warning(f"Could not extract name from author profile: {author_url}")
                return None
            ...
            logger.error(f"Error scraping author profile {author_id}: {str(e)}", exc_info=True)
            return None
    ...
    def scrape_article(self, article_url: str, article_id: str) -> Optional[Dict]:
        ...
        if article_date:
            ...
            if article_date < one_year_ago:
                ...
                logger.debug(f"Article {article_id} is older than 1 year - skipping related articles extraction")
        ...
        try:
            ...
            if not article_will_be_skipped:
                ...
                try:
                    ...
                    if r.status_code == 404:
                        logger.warning(f"Article {article_id} not found (404) when extracting related articles - likely deleted or moved")
                    ...
                    try:
                        ...
                        if related_date:
                            ...
                            if related_date < one_year_ago:
                                logger.debug(f"Skipping related article {related['id']} - older than 1 year ({related_date.date()})")
                                continue
                        ...
                        logger.error(f"Error saving related article {related['id']}: {str(e)}")
            else:
                ...
                try:
                    ...
                    # Handle 404 errors gracefully (article deleted/moved)
                    if r.status_code == 404:
                        logger.warning(f"Article {article_id} not found (404) when extracting authors - likely deleted or moved")
                        return article_data  # Return what we have so far
                    r.raise_for_status()
                    soup = BeautifulSoup(r.text, 'html.parser')
                except requests.exceptions.HTTPError as e:
            ...
            if authors:
                ...
                for author in authors:
                    ...
                    parsed_authors = self.author_normalizer.parse_author_string(author_name)
                    if parsed_authors:
                ...
                if author_string:
                    ...
                    if parent_author:
                        ...
                        # Split the author string into individual authors and process them
                        individual_author_names = self._split_author_names(parent_author)
                        for author_name in individual_author_names:
                            if author_name:
                                # Clean the name
                                cleaned_name = self._clean_author_name(author_name)
            ...
            for author in authors:
                try:
                    ...
                    for parsed_author in parsed_authors:
                        ...
                        if parsed_author.location and parsed_author.location not in article_locations:
                        ...
                        if 'id' in author and author.get('id'):
                            ...
                            if not self.db.author_exists(author_id=author_id):
                                ...
                                if normalized_name and self.db.get_author_by_name_or_alias(name=normalized_name):
                                ...
                                is_employed = self._is_author_currently_employed(normalized_name)
                                if is_employed:
                                    ...
                                    try:
                                        ...
                                        if existing_author:
                                            ...
                                            existing_author.alias = ', '.join(alias_list)
                                            session.commit()
                                    finally:
                                        session.close()
                        else:
                            ...
                            normalized = normalized_name.strip()  # Normalized without location
                            ...
                            existing_author = self.db.get_author_by_name_or_alias(name=normalized, alias=normalized)
                            
                            if not existing_author:
                                # Check if author exists by name or alias
                                if not self.db.author_exists(name=normalized, alias=normalized):
                                    ...
                                    try:
                                        ...
                                        logger.debug(f"Saved author without link: {normalized} (alias: {alias_str}, employed: False)")
                                    except Exception as e:
                                        logger.error(f"Error saving author without link {normalized}: {str(e)}")
                                else:
                                    ...
                                    if existing_author:
                                        session = self.db.Session()
                                        try:
                                            fresh_author = session.query(Author).filter_by(name=normalized).first()
                                            if fresh_author:
                                                # Update alias
                                                alias_list = []
                                                if fresh_author.alias:
                                                    alias_list = [a.strip() for a in fresh_author.alias.split(',')]
                                                for part in alias_parts:
                                                    if part not in alias_list:
                                                        alias_list.append(part)
                                                fresh_author.alias = ', '.join(alias_list)
                                                session.commit()
                                        finally:
                                            session.close()
                        ...
                        if existing_author:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if author_name_to_query:
                                        fresh_author = session.query(Author).filter_by(name=author_name_to_query).first()
                                        if fresh_author:
                            ...
                            if author_id_to_link:
                                self.db.link_article_to_author(article_id, author_id_to_link)
                                ...
                                logger.debug(f"Author {full_name} has no ID, cannot link to article via association table")
                except Exception as e:
            ...
            logger.error(f"Error extracting related articles/authors for {article_id}: {str(e)}", exc_info=True)
            ...
            try:
                ...
                try:
                    ...
                    if author_id and author_id.startswith('no-id-'):
                        logger.debug(f"[{thread_short}] Skipping placeholder author ID: {author_id}")
                        skipped += 1
                    ...
                    if self.db.author_exists(author_id=author_id):
                        logger.debug(f"[{thread_short}] Author {author_id} already exists (by ID), skipping")
                        skipped += 1
                        self.author_queue.task_done()
                        continue
                    ...
                    if author_data:
                        ...
                        if author_name:
                            existing_by_name = self.db.get_author_by_name_or_alias(name=author_name)
                            if existing_by_name:
                                ...
                                if existing_by_name.author_id and not existing_by_name.author_id.startswith('no-id-'):
                                    # Existing author has real ID, skip this one
                                    logger.debug(f"[{thread_short}] Author '{author_name}' already exists with ID {existing_by_name.author_id}, skipping")
                                    skipped += 1
                                    self.author_queue.task_done()
                                    continue
                                    ...
                                    try:
                                        ...
                                        existing_by_name.author_url = author_data.get('url')
                                        if author_data.get('title'):
                                            existing_by_name.title = author_data.get('title')
                                        if author_data.get('alternate_name'):
                                            existing_by_name.alternate_name = author_data.get('alternate_name')
                                        if author_data.get('bio'):
                                            existing_by_name.bio = author_data.get('bio')
                                        if author_data.get('image_url'):
                                            existing_by_name.image_url = author_data.get('image_url')
                                        is_employed = self._is_author_currently_employed(author_name)
                                        existing_by_name.currently_employed = 1 if is_employed else 0
                                        session.commit()
                                        successful += 1
                                        logger.info(f"[{thread_short}] ✓ Updated author '{author_name}' with real ID {author_id}")
                                    finally:
                                        session.close()
                                    self.author_queue.task_done()
                                    continue
                        ...
                        if not self.db.author_exists(author_id=author_id, name=author_name):
                            ...
                            successful += 1
                            logger.info(f"[{thread_short}] ✓ Author {author_data['name']} ({author_id})")
                        else:
                            # Author exists, update currently_employed status
                            session = self.db.Session()
                            try:
                                ...
                                if existing_author:
                                    ...
                                    if author_data.get('alternate_name'):
                                        existing_author.alternate_name = author_data.get('alternate_name')
                                    if author_data.get('bio'):
                                        existing_author.bio = author_data.get('bio')
                                    if author_data.get('image_url'):
                                        existing_author.image_url = author_data.get('image_url')
                                    existing_author.currently_employed = 1 if is_employed else 0
                                    session.commit()
                                    successful += 1
                                    logger.info(f"[{thread_short}] ✓ Updated author {author_data['name']} ({author_id})")
                            finally:
                                session.close()
                    else:
                        failed += 1
                        logger.warning(f"[{thread_short}] ✗ Author {author_id}: Failed to scrape")
                    ...
                    logger.error(f"[{thread_short}] ✗ Author {author_id}: {str(e)}", exc_info=True)
                    self.author_queue.task_done()
                    
            except Exception as e:
                logger.error(f"[{thread_name}] Error in author scraping worker: {str(e)}", exc_info=True)
        
        logger.info(f"[{thread_short}] Author worker done: {successful}✓ {failed}✗ {skipped}⊘")
    ...
    def process_author_queue(self):
        ...
        logger.info(f"Starting parallel author profile scraping with {self.num_author_worker_threads} threads...")
    ...
    def link_articles_to_authors_after_scraping(self):
        ...
        try:
            ...
            for article in articles:
                ...
                individual_author_names = self._split_author_names(cleaned_author_string)
                ...
                for author_name in individual_author_names:
                    ...
                    if author_lower in name_to_author:
                        ...
                        if ',' in cleaned_name:
                            ...
                            existing_author = self.db.get_author_by_name_or_alias(name=cleaned_name)
                            if existing_author:
                                author = existing_author
                    
                    # If author found, create association
                    if author and author.author_id:
                        if self.db.link_article_to_author(article.article_id, author.author_id):
                            linked_count += 1
                            logger.debug(f"Linked article {article.article_id} to author {author.author_id} ({author.name})")
        except Exception as e:
    ...
    def run(self, headless: bool = False, clean_articles: bool = False, 
            clean_authors: bool = False, clean_related_articles: bool = False, 
        ...
        try:
            ...
            if any([clean_articles, clean_authors, clean_related_articles, clean_article_author_associations]):
            ...
            logger.info("="*80)
            logger.info(f"STEP 1 COMPLETE: Impressum authors loaded and scraped ({len(self.currently_employed_authors)} authors)")
            logger.info("="*80)
            ...
            logger.info("="*80)
            logger.info("STEP 2: Starting article scraping (will analyze authors and related articles)...")
            logger.info("="*80)
            ...
            logger.info("="*80)
            logger.info(f"Starting parallel author scraping threads ({self.num_author_worker_threads} workers)...")
            logger.info("="*80)
            import threading
            self.author_scraping_active = True
            self.author_scraping_threads = []
            ...
            logger.info(f"Started {self.num_author_worker_threads} author scraping threads (running in parallel with article scraping)")
            ...
            logger.info("Article scraping complete, waiting for author scraping to finish...")
            ...
            logger.info("Author scraping complete (6 workers processed authors in parallel)")
