...
def build_network(articles):
    ...
    logger.info(
        f"Added {edges_added} edges, skipped {edges_skipped_orphan} orphan edges"
    )
    return G
...
def analyze_connectivity(G):
    ...
    print(
        f"Largest component size: {component_sizes[0] if component_sizes else 0:,} nodes"
    )
    print(
        f"Percentage in largest component: {component_sizes[0] / num_nodes * 100 if num_nodes > 0 else 0:.2f}%"
    )
    ...
    if components:
        ...
        print(
            f"Average degree: {2 * len(largest_component.edges()) / len(largest_component.nodes()) if len(largest_component.nodes()) > 0 else 0:.2f}"
        )
        ...
        if len(largest_component.nodes()) <= 10000:
            ...
            try:
                ...
                print(
                    f"Average shortest path length (sample of 1000 nodes): {avg_path_length:.2f}"
                )
            except Exception as e:
    ...
    print(
        f"Median degree: {sorted(degree_values)[len(degree_values)//2] if degree_values else 0}"
    )
    ...
    print(
        f"Largest component: {component_sizes[0] if component_sizes else 0:,} nodes ({component_sizes[0]/num_nodes*100 if num_nodes > 0 and component_sizes else 0:.2f}%)"
    )
    print(
        f"Isolated nodes: {len(isolated):,} ({len(isolated)/num_nodes*100 if num_nodes > 0 else 0:.2f}%)"
    )
    print(f"Average degree: {2 * num_edges / num_nodes if num_nodes > 0 else 0:.2f}")
