...
def load_articles_from_db(db_path):
    ...
    try:
        articles = (
            session.query(Article).filter(Article.related_articles.isnot(None)).all()
        )
...
def build_network(articles):
    ...
    for related_id in orphan_nodes:
        G.add_node(related_id, title="", category="", article_date="", in_table=False)

    logger.info(
        f"Added {len(orphan_nodes)} orphan nodes (IDs in related_articles but not in table)"
    )
...
def visualize_network_pyvis(G, output_path, height="1000px", width="100%"):
    ...
    nodes_orphan = [
        node for node in G.nodes() if not G.nodes[node].get("in_table", True)
    ]
...
def visualize_network_plotly(G, output_path):
    ...
    node_size_table = [
        min(10 + degrees.get(node, 0) * 2, 50) for node in nodes_in_table
    ]

    node_trace_table = go.Scatter(
        x=node_x_table,
        y=node_y_table,
        mode="markers+text",
        name="Articles in table",
        marker=dict(
            size=node_size_table, color="#4A90E2", line=dict(width=1, color="white")
        ),
    ...
    if nodes_orphan:
        ...
        node_text_orphan = [
            f"ID: {node}<br>(Not in table)<br>Connections: {degrees.get(node, 0)}"
            for node in nodes_orphan
        ]
        node_size_orphan = [
            min(5 + degrees.get(node, 0) * 1.5, 30) for node in nodes_orphan
        ]

        node_trace_orphan = go.Scatter(
            x=node_x_orphan,
            y=node_y_orphan,
            mode="markers+text",
            name="Orphan nodes",
            marker=dict(
                size=node_size_orphan,
                color="#E24A4A",
        ...
        fig = go.Figure(
            data=[edge_trace, node_trace_table, node_trace_orphan],
            layout=go.Layout(
        ...
        fig = go.Figure(
            data=[edge_trace, node_trace_table],
            layout=go.Layout(
