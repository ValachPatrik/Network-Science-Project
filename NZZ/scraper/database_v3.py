...
class AuthorRaw(Base):
    ...
    author_id = Column(String(255), unique=True, nullable=True, index=True)  # ID from URL or temp ID (unique when not null)
...
class DatabaseManagerV3:
            ...
            with self.engine.connect() as conn:
                ...
                if not any('author_id' in idx for idx in indexes):
                    try:
                        ...
                        """))
                        conn.commit()
                        logger.info("Created unique index on authors_raw.author_id (with WHERE clause)")
                    except Exception:
                        ...
                        try:
                            ...
                            """))
                            conn.commit()
                            logger.info("Created unique index on authors_raw.author_id (allows multiple NULLs)")
                        except Exception as e2:
                            logger.debug(f"Could not create unique index on author_id: {str(e2)}")
                            
        except Exception as e:
            logger.debug(f"Could not ensure unique constraints: {str(e)}")
            # Continue anyway - the Column unique=True should handle it
    
    def save_author_raw(self, author_id: Optional[str], name: str, title: Optional[str] = None,
                       alt_name: Optional[str] = None, bio: Optional[str] = None,
                       author_url: Optional[str] = None, alias: Optional[str] = None,
                       has_info: int = 0, department: Optional[str] = None) -> AuthorRaw:
        ...
        try:
            ...
            if not author_id:
                existing = session.query(AuthorRaw).filter_by(name=name, author_id=None).first()
                if existing:
    ...
    def save_article_raw(self, article_id: str, title: str, content: str,
                        tags: Optional[str] = None, category: Optional[str] = None,
                        article_url: str = None, article_date: Optional[datetime] = None,
                        article_updated: Optional[datetime] = None, author: Optional[str] = None,
                        author_links: Optional[str] = None, description: Optional[str] = None,
                        related_articles: Optional[str] = None) -> ArticleRaw:
        ...
        try:
            ...
            existing = session.query(ArticleRaw).filter_by(article_id=article_id).first()
            ...
            if existing:
                ...
                existing.related_articles = related_articles or existing.related_articles
                existing.scraped_at = datetime.utcnow()
                session.commit()
                return existing
            else:
    ...
    def article_raw_exists(self, article_id: str) -> bool:
        ...
        session = self.Session()
        try:
            return session.query(ArticleRaw).filter_by(article_id=article_id).first() is not None
        finally:
            session.close()
    
    def author_raw_exists(self, author_id: Optional[str] = None, name: Optional[str] = None) -> bool:
        """Check if author raw exists."""
        session = self.Session()
        try:
            if author_id:
                return session.query(AuthorRaw).filter_by(author_id=author_id).first() is not None
            if name:
    ...
    def clean_database(self, clean_authors_raw: bool = False, clean_articles_raw: bool = False):
