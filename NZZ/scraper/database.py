...
class Article(Base):
    ...
    )  # Category from URL (e.g., zuerich, wirtschaft)
    scraped_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    article_url = Column(String(500), nullable=False)
    article_date = Column(
        DateTime, nullable=True
    )  # Date from the article itself (published date)
    ...
    location = Column(
        String(200), nullable=True
    )  # Location associated with article (from author location, e.g., "Bangkok", "Mumbai")
...
class RelatedArticle(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    article_id = Column(
        String(255), ForeignKey("articles.article_id"), nullable=False, index=True
    )
...
class Author(Base):
    ...
    )  # ID from URL (e.g., ld.1894615), nullable for authors without links
    name = Column(String(200), nullable=False)
    title = Column(
        String(200), nullable=True
    )  # Job title (e.g., "Ressortleiter Feuilleton")
    ...
    author_url = Column(
        String(500), nullable=True
    )  # Full URL to author profile, nullable for authors without links
    alias = Column(
        String(200), nullable=True
    )  # Manual alias (e.g., for cities like "Andreas Babst, Bangkok")
    currently_employed = Column(
        Integer, default=0, nullable=False
    )  # 1 if currently employed, 0 if not
    scraped_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    # Many-to-many relationship with articles
    articles = relationship(
        "Article", secondary=article_author_association, backref="authors"
    )
...
class DatabaseManager:
    ...
    def _migrate_database(self):
        ...
        try:
            ...
            try:
                author_columns = {
                    col["name"]: col for col in inspector.get_columns("authors")
                }
                ...
                if "alias" not in author_columns:
                    try:
                        with self.engine.connect() as conn:
                            conn.execute(
                                text(
                                    "ALTER TABLE authors ADD COLUMN alias VARCHAR(200)"
                                )
                            )
                            conn.commit()
                ...
                if "currently_employed" not in author_columns:
                    try:
                        with self.engine.connect() as conn:
                            conn.execute(
                                text(
                                    "ALTER TABLE authors ADD COLUMN currently_employed INTEGER DEFAULT 0"
                                )
                            )
                            conn.commit()
                            logger.info(
                                "Added 'currently_employed' column to authors table"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Could not add 'currently_employed' column: {str(e)}"
                        )
                ...
                if "author_id" in author_columns and not author_columns[
                    "author_id"
                ].get("nullable", True):
                    logger.info(
                        "Note: author_id column is NOT NULL - using 'no-id' placeholder for authors without links"
                    )
            except Exception as e:
                ...
                if "author_ids" not in article_columns:
                    try:
                        with self.engine.connect() as conn:
                            conn.execute(
                                text(
                                    "ALTER TABLE articles ADD COLUMN author_ids VARCHAR(500)"
                                )
                            )
                            conn.commit()
                ...
                if "location" not in article_columns:
                    try:
                        with self.engine.connect() as conn:
                            conn.execute(
                                text(
                                    "ALTER TABLE articles ADD COLUMN location VARCHAR(200)"
                                )
                            )
                            conn.commit()
    ...
    def article_exists(self, article_id: str) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                result = (
                    session.query(Article).filter_by(article_id=article_id).first()
                    is not None
                )
                return result
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Failed to check if article {article_id} exists after {max_retries} attempts: {str(e)}"
                        )
                        return False
                else:
            ...
            try:
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        # Last attempt failed
                        logger.error(
                            f"Failed to save article {article_id} after {max_retries} attempts: {str(e)}"
                        )
                        raise
                else:
                    # Different OperationalError - don't retry
    ...
    def get_oldest_article_date(self) -> datetime:
        ...
        try:
            ...
            oldest = (
                session.query(Article.article_date)
                .filter(Article.article_date.isnot(None))
                .order_by(Article.article_date.asc())
                .first()
            )
            ...
            try:
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        # Last attempt failed
                        logger.error(
                            f"Failed to get article {article_id} after {max_retries} attempts: {str(e)}"
                        )
                        raise
                else:
                    # Different OperationalError - don't retry
    ...
    def save_related_article(
        self, article_id: str, related_article_id: str, related_article_url: str
    ) -> RelatedArticle:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                existing = (
                    session.query(RelatedArticle)
                    .filter_by(
                        article_id=article_id, related_article_id=related_article_id
                    )
                    .first()
                )
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger = logging.getLogger("nzz_scraper")
                        logger.error(
                            f"Failed to save related article {related_article_id} after {max_retries} attempts: {str(e)}"
                        )
                        raise
                else:
                    # Different OperationalError - don't retry
    ...
    def author_exists(
        self, author_id: str = None, name: str = None, alias: str = None
    ) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if author_id:
                    result = (
                        session.query(Author).filter_by(author_id=author_id).first()
                        is not None
                    )
                    if result:
                        return True
                ...
                if name:
                    ...
                    result = (
                        session.query(Author).filter_by(name=name).first() is not None
                    )
                    if result:
                        return True
                    ...
                    for author in all_authors:
                        ...
                        if author.alias:
                            ...
                            alias_parts = [
                                a.strip().lower() for a in author.alias.split(",")
                            ]
                ...
                if alias:
                    ...
                    result = (
                        session.query(Author).filter_by(alias=alias).first() is not None
                    )
                    if result:
                        return True
                    # Check if alias matches any name
                    result = (
                        session.query(Author).filter(Author.name == alias).first()
                        is not None
                    )
                    if result:
                        return True
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Failed to check if author exists after {max_retries} attempts: {str(e)}"
                        )
                        return False
                else:
                    # Different OperationalError - don't retry
    ...
    def get_author_by_name_or_alias(
        self, name: str = None, alias: str = None
    ) -> Optional[Author]:
            ...
            try:
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger = logging.getLogger("nzz_scraper")
                        logger.warning(
                            f"Failed to get author by name/alias after {max_retries} attempts: {str(e)}"
                        )
                        return None
                else:
                    # Different OperationalError - don't retry
            ...
            try:
                ...
                if author_id:
                    ...
                    existing = (
                        ...
                        .filter_by(name=name, author_id=placeholder_id)
                        .first()
                    )
                    if not existing:
                ...
                if existing:
                    ...
                    if name:
                        existing.name = (
                            name[:200] if len(name) <= 200 else name[:197] + "..."
                        )
                    if title:
                        existing.title = (
                            title[:200] if len(title) <= 200 else title[:197] + "..."
                        )
                    if alternate_name:
                        existing.alternate_name = (
                            alternate_name[:100]
                            if len(alternate_name) <= 100
                            else alternate_name[:97] + "..."
                        )
                    if bio:
                        # Clean bio
                    ...
                    existing.alias = (
                        ", ".join(alias_list)[:200]
                        if len(", ".join(alias_list)) <= 200
                        else ", ".join(alias_list)[:197] + "..."
                    )
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        logger = logging.getLogger("nzz_scraper")
                        logger.error(
                            f"Failed to save author {name} after {max_retries} attempts: {str(e)}"
                        )
                        raise
                else:
                    # Different OperationalError - don't retry
            ...
            try:
                ...
                if "database is locked" in error_str or "locked" in error_str:
                    if attempt < max_retries - 1:
                        ...
                        # Last attempt failed
                        logger.error(
                            f"Failed to link article {article_id} to author {author_id} after {max_retries} attempts: {str(e)}"
                        )
                        raise
                else:
                    # Different OperationalError - don't retry
    ...
    def clean_database(
        ...
        try:
            ...
            if clean_related_articles:
                deleted_counts["related_articles"] = session.query(
                    RelatedArticle
                ).delete()
            else:
    ...
    def clean_test_data(self, article_ids: List[str] = None):
        ...
        try:
            ...
            if article_ids:
                ...
                deleted_counts["related_articles"] = (
                    session.query(RelatedArticle)
                    .filter(RelatedArticle.article_id.in_(article_ids))
                    .delete(synchronize_session=False)
                )
                ...
                deleted_counts["article_author_associations"] = session.execute(
                    text(
                        f"DELETE FROM article_author_association WHERE article_id IN (SELECT id FROM articles WHERE article_id IN ({article_id_placeholders}))"
                    )
                ).rowcount

                # Delete articles
                deleted_counts["articles"] = (
                    session.query(Article)
                    .filter(Article.article_id.in_(article_ids))
                    .delete(synchronize_session=False)
                )
