...
def view_articles(limit: int = None, article_id: str = None, search: str = None):
    ...
    try:
        ...
        if article_id:
            ...
            articles = [
                a
                for a in db.get_all_articles()
                if search_lower in (a.title or "").lower()
                or search_lower in (a.content or "").lower()
                or search_lower in (a.article_url or "").lower()
                or search_lower in (a.category or "").lower()
                or search_lower in (a.author or "").lower()
            ]
            if not articles:
        ...
        for idx, article in enumerate(articles, 1):
            ...
            if article.article_date:
                print(
                    f"  Published: {article.article_date.strftime('%Y-%m-%d %H:%M:%S')}"
                )
            else:
                print(f"  Published: Not available")
            if article.article_updated:
                print(
                    f"  Updated: {article.article_updated.strftime('%Y-%m-%d %H:%M:%S')}"
                )
        ...
        if not limit and not article_id and not search:
            ...
            if oldest_date:
                print(
                    f"Oldest article date: {oldest_date.strftime('%Y-%m-%d %H:%M:%S')}"
                )
        print(f"{'='*80}\n")
...
def show_articles_per_day_histogram():
    ...
    try:
        ...
        print(
            f"Average articles per day: {len(articles_with_dates) / len(sorted_days):.2f}"
        )
        ...
        try:
            ...
            dates = [
                datetime.combine(date, datetime.min.time()) for date in sorted_days
            ]
            ...
            print("\n" + "-" * 80)
            print(
                "Note: Install matplotlib for a visual histogram: pip install matplotlib"
            )
...
def list_articles_summary():
    ...
    try:
        ...
        for article in articles:
            ...
            date_str = (
                article.scraped_at.strftime("%Y-%m-%d") if article.scraped_at else "N/A"
            )
            ...
            print(
                f"{article.article_id[:28]:<30} {title_short:<40} {category[:13]:<15} {date_str:<20} {status:<10}"
            )
...
def show_dataframe(limit: int = None, article_id: str = None, search: str = None):
    ...
    try:
        ...
        for article in articles:
            ...
            # Format dates
            published = (
                article.article_date.strftime("%Y-%m-%d %H:%M:%S")
                if article.article_date
                else None
            )
            updated = (
                article.article_updated.strftime("%Y-%m-%d %H:%M:%S")
                if article.article_updated
                else None
            )
            scraped = (
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command == "list" or command == "summary":
            ...
            print("Usage:")
            print(
                "  python view_articles.py list              - List all articles in summary"
            )
            print(
                "  python view_articles.py df                - Show all articles as DataFrame"
            )
            print(
                "  python view_articles.py df --limit N      - Show first N articles as DataFrame"
            )
            print(
                "  python view_articles.py df --id ID         - Show specific article as DataFrame"
            )
            print(
                "  python view_articles.py df --search TERM   - Search articles and show as DataFrame"
            )
            print(
                "  python view_articles.py view               - View all articles in detail"
            )
            print("  python view_articles.py view --limit N    - View first N articles")
            print(
                "  python view_articles.py view --id ID       - View specific article by ID"
            )
            print(
                "  python view_articles.py view --search TERM - Search articles by term"
            )
            print(
                "  python view_articles.py oldest             - Show oldest article date"
            )
            print(
                "  python view_articles.py oldest-articles    - Show top 10 oldest articles"
            )
            print(
                "  python view_articles.py oldest-articles --limit N - Show top N oldest articles"
            )
            print(
                "  python view_articles.py histogram          - Show histogram of articles per day"
            )
    else:
