...
def delete_articles_by_date(db: DatabaseManager, before_date: Optional[datetime] = None,
                            after_date: Optional[datetime] = None, confirm: bool = True) -> int:
...
def delete_articles_by_category(db: DatabaseManager, category: str, confirm: bool = True) -> int:
...
def delete_duplicates(db: DatabaseManager, confirm: bool = True) -> int:
    ...
    duplicates_by_id = db.session.query(
        Article.article_id,
        db.session.query(Article).filter_by(article_id=Article.article_id).count().label('count')
    ).having(db.session.query(Article).filter_by(article_id=Article.article_id).count() > 1).all()
    
    # Find duplicates by article_url
    duplicates_by_url = db.session.query(
        Article.article_url,
        db.session.query(Article).filter_by(article_url=Article.article_url).count().label('count')
    ).having(db.session.query(Article).filter_by(article_url=Article.article_url).count() > 1).all()
    ...
    for article_id, _ in duplicates_by_id:
        articles = db.session.query(Article).filter_by(article_id=article_id).order_by(Article.scraped_at).all()
        if len(articles) > 1:
    ...
    for article_url, _ in duplicates_by_url:
        articles = db.session.query(Article).filter_by(article_url=article_url).order_by(Article.scraped_at).all()
        if len(articles) > 1:
...
def reset_database(db: DatabaseManager, confirm: bool = True) -> bool:
    ...
    if confirm:
        ...
        print(f"This will delete ALL {count} articles and recreate the database schema.")
        response = input("Are you sure you want to continue? (yes/no): ").strip().lower()
        if response not in ['yes', 'y']:
            print("Operation cancelled.")
            return False
...
Examples:
  ...
  python clean_db.py --reset
    ...
    parser.add_argument('--delete-before-days', type=int, metavar='DAYS',
                       help='Delete articles scraped before N days ago')
    parser.add_argument('--delete-after-days', type=int, metavar='DAYS',
                       help='Delete articles scraped after N days ago')
    parser.add_argument('--delete-category', type=str, metavar='CATEGORY',
    ...
    try:
        ...
        if not args.stats and (args.delete_all or args.delete_before_days or 
                              args.delete_after_days or args.delete_category or 
                              args.delete_duplicates or args.reset):
        ...
        if args.delete_before_days:
            ...
            delete_articles_by_date(db, before_date=before_date, confirm=not args.no_confirm)
        
        if args.delete_after_days:
            after_date = datetime.utcnow() - timedelta(days=args.delete_after_days)
            delete_articles_by_date(db, after_date=after_date, confirm=not args.no_confirm)
        
        if args.delete_category:
            delete_articles_by_category(db, args.delete_category, confirm=not args.no_confirm)
