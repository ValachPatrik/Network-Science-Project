...
class RelatedArticlesFilter:
    ...
    def load_data(self, chunk_size=10000):
        ...
        while offset < total_count:
            ...
            logger.info(
                f"Loading chunk: rows {offset:,} to {min(offset + chunk_size, total_count):,}..."
            )
    ...
    def filter_related_articles(self, df, valid_ids):
        ...
        def filter_related(row):
            ...
            try:
                ...
                filtered = [
                    article_id for article_id in parsed if article_id in valid_ids
                ]
                return filtered
            except Exception as e:
                logger.warning(
                    f"Error parsing related_articles for article_id {row['article_id']}: {e}"
                )
                return []
        ...
        total_filtered = sum(
            len(row["related_articles_filtered"]) for _, row in df.iterrows()
        )
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if not column_exists:
                    logger.info(
                        "Adding 'related_articles_filtered' column to articles table..."
                    )
                    alter_query = text(
                        """
    ...
    def update_database(self, df, batch_size=1000):
        ...
        df["related_articles_filtered_str"] = df["related_articles_filtered"].apply(
            list_to_json_string
        )
            ...
            with self.engine.begin() as conn:  # Use begin() for transaction management
                for i in range(0, total_rows, batch_size):
                    ...
                    if updated % (batch_size * 10) == 0:
                        logger.info(
                            f"Updated {updated:,} / {total_rows:,} rows ({updated/total_rows*100:.1f}%)"
                        )
