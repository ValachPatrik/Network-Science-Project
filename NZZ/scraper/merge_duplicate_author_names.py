...
class AuthorMerger:
    ...
    def find_duplicates(self, df):
        ...
        duplicates = {
            name: indices for name, indices in groups.items() if len(indices) > 1
        }
        ...
        for val in values:
            if pd.notna(val) and val:
                ...
                if val_str:
                    ...
                    items = [
                        item.strip()
                        for item in val_str.split(delimiter)
                        if item.strip()
                    ]
    ...
    def _merge_bio(self, *bios):
        ...
        """
        valid_bios = [
            str(bio).strip() for bio in bios if pd.notna(bio) and str(bio).strip()
        ]
        if not valid_bios:
            return None
    ...
    def merge_author_data(self, author_rows):
        ...
        title_vals = [
            a["title"]
            for a in has_info_1_authors
            if pd.notna(a["title"]) and a["title"]
        ]
        if not title_vals:
            title_vals = [
                a["title"]
                for a in has_info_0_authors
                if pd.notna(a["title"]) and a["title"]
            ]
        ...
        alt_name_vals_1 = [
            a["alt_name"]
            for a in has_info_1_authors
            if pd.notna(a["alt_name"]) and a["alt_name"]
        ]
        alt_name_vals_0 = [
            a["alt_name"]
            for a in has_info_0_authors
        ...
        alias_vals_1 = [
            a["alias"]
            for a in has_info_1_authors
            if pd.notna(a["alias"]) and a["alias"]
        ]
        alias_vals_0 = [
            a["alias"]
            for a in has_info_0_authors
        ...
        dept_vals_1 = [
            a["department"]
            for a in has_info_1_authors
            if pd.notna(a["department"]) and a["department"]
        ]
        dept_vals_0 = [
            a["department"]
            for a in has_info_0_authors
        ...
        merged_department = (
            dept_vals_1[0]
            if dept_vals_1
            else (dept_vals_0[0] if dept_vals_0 else primary["department"])
        )
        ...
        loc_vals_1 = [
            a["location"]
            for a in has_info_1_authors
            if pd.notna(a["location"]) and a["location"]
        ]
        loc_vals_0 = [
            a["location"]
            for a in has_info_0_authors
        ...
        merged_location = (
            loc_vals_1[0]
            if loc_vals_1
            else (loc_vals_0[0] if loc_vals_0 else primary["location"])
        )
        ...
        if pd.notna(primary["author_id"]) and primary["author_id"]:
            ...
            author_id_vals_1 = [
                a["author_id"]
                for a in has_info_1_authors
                if pd.notna(a["author_id"]) and a["author_id"]
            ]
            author_id_vals_0 = [
                a["author_id"]
                for a in has_info_0_authors
    ...
    def check_author_id_conflict(
        self, conn, author_id, current_author_id, duplicate_ids=None
    ):
    ...
    def update_author(self, merged_data, conn=None, duplicate_ids=None):
        ...
        if conn is not None and author_id and pd.notna(author_id):
            if self.check_author_id_conflict(
                conn, author_id, current_id, duplicate_ids
            ):
                ...
                get_original_query = text(
                    "SELECT author_id FROM authors WHERE id = :id"
                )
                ...
                if original_author_id and pd.notna(original_author_id):
                    ...
                    logger.warning(
                        f"Author ID conflict detected for author {current_id}. "
        ...
        try:
            with self.engine.begin() as conn:  # Use transaction
                for name_normalized, indices in duplicates.items():
                    ...
                    update_query, update_params = self.update_author(
                        merged_data, conn=conn, duplicate_ids=duplicate_ids
                    )
    ...
    def run(self, dry_run=False):
        ...
        try:
            ...
            if dry_run:
                ...
                for name_normalized, indices in list(duplicates.items())[
                    :10
                ]:  # Show first 10
                    author_rows = [df.loc[idx] for idx in indices]
                    logger.info(
                    ...
                    for row in author_rows:
                        logger.info(
                            f"    ID: {row['id']}, has_info: {row['has_info']}, "
