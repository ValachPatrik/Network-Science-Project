...
class AuthorNormalizer:
    ...
    def __init__(
        ...
        if use_llm:
            try:
                ...
                self.llm_classifier = LLMLocationClassifier(
                    model_name=model_name, use_llm=True
                )
                if self.llm_classifier.use_llm:
                    logger.info(
                        "LLM classifier initialized for location/name detection"
                    )
                else:
                    logger.warning(
                        "LLM classifier not available - will use geopy/heuristics"
                    )
            except ImportError:
                logger.warning(
                    "llm_classifier module not found. LLM classification disabled."
                )
    ...
    def is_location(self, term: str, context: Optional[str] = None) -> bool:
        ...
        if len(words) == 2:
            ...
            has_location_keyword = any(
                kw in term_lower
                for kw in ["valley", "city", "town", "gazastreifen", "gaza"]
            )

            # If it looks like a name (both capitalized, no location indicators), be more conservative
            if (
                both_capitalized
                and not has_location_connector
                and not has_location_keyword
            ):
        ...
        if self.llm_classifier and self.llm_classifier.use_llm:
            try:
                ...
                for loc in locations:
                    ...
                    if loc_lower in term_lower or any(
                        word in term_lower for word in loc_lower.split()
                    ):
                ...
                if valid_locations:
                    ...
                    for name in names:
                        ...
                        if name_lower in term_lower or any(
                            word in term_lower for word in name_lower.split()
                        ):
                    ...
                    if (
                        valid_names
                        and len(words) == 2
                        and both_capitalized
                        and not has_location_connector
                    ):
                        # Likely a name, not a location
                        logger.debug(
                            f"LLM found both names and locations for '{term_clean}', prioritizing name (found: {valid_names})"
                        )
                        self.location_cache[term_lower] = False
                        return False

                    logger.debug(
                        f"LLM classified '{term_clean}' as location (found: {valid_locations})"
                    )
                ...
                if valid_names and not valid_locations:
                    ...
                    logger.debug(
                        f"LLM classified '{term_clean}' as name (found: {valid_names})"
                    )
                    self.location_cache[term_lower] = False
                    return False
                ...
                logger.debug(
                    f"LLM classification error for '{term_clean}': {str(e)}, falling back to geopy"
                )
        ...
        if self.use_geopy and self.geocoder:
            try:
                ...
                location = self.geocoder.geocode(
                    term_clean, exactly_one=True, timeout=3
                )
                if location:
                    ...
                    if (
                        "place" in location.address.lower()
                        or "city" in location.address.lower()
                    ):
                        self.location_cache[term_lower] = True
                        return True
    ...
    def is_department(self, term: str, context: Optional[str] = None) -> bool:
        ...
        if self.llm_classifier and self.llm_classifier.use_llm:
            try:
                is_dept = self.llm_classifier.is_department(
                    term_clean, context, min_confidence=0.6
                )
                if is_dept:
                ...
                logger.debug(
                    f"LLM classification error for '{term_clean}': {str(e)}, falling back to heuristics"
                )
            ...
            if " und " in author_string.lower() or " and " in author_string.lower():
                ...
                if len(comma_parts) == 2:
                    ...
                    if name_part and locations_part:
                        ...
                        location_list = re.split(
                            r"\s+und\s+|\s+and\s+", locations_part, flags=re.IGNORECASE
                        )
                        location_list = [
                        ...
                        name_looks_valid = len(name_words) >= 2 and all(
                            w[0].isupper() if w else False for w in name_words[:2]
                        )
                        ...
                        if name_looks_valid and location_list:
                            ...
                            if validated_locations:
                                ...
                                author = self._parse_name_to_author(
                                    name_part, primary_location, None, author_string
                                )
                                if author:
                                    return [author]
                ...
                if len(parts) == 2:
                    ...
                    if location_part and name_part:
                        ...
                        if name_looks_valid:
                            ...
                            has_location_keyword = any(
                                kw in location_part.lower() for kw in location_keywords
                            )
                            ...
                            has_location_connector = any(
                                conn in location_part.lower()
                                for conn in location_connectors
                            )

                            # 3. Multi-word capitalized (likely city name)
                            is_multi_word_capitalized = len(location_words) > 1 and all(
                                w[0].isupper() if w else False for w in location_words
                            )

                            # 4. Single word capitalized (likely city name)
                            is_single_word_capitalized = (
                                len(location_words) == 1
                                and location_part[0].isupper()
                                and len(location_part) >= 3
                            )

                            # 5. Logic-based detection
                            is_location_confirmed = self.is_location(
                                location_part, context=author_string
                            )

                            # If ANY of these indicators are true, treat as location and split
                            if (
                                has_location_keyword
                                or has_location_connector
                                or is_location_confirmed
                                or (
                                    is_multi_word_capitalized
                                    and not self.is_department(
                                        location_part, context=author_string
                                    )
                                )
                                or (
                                ...
                                author = self._parse_name_to_author(
                                    name_part, location_part, None, author_string
                                )
                                if author:
                                    return [author]
        ...
        if "," not in author_string:  # Only check for standalone locations if no comma
            ...
            if len(words) == 1:
                ...
                if not is_name_with_prefix:
                    ...
                    has_location_connector = any(
                        connector in author_string.lower()
                        for connector in [
                            " de ",
                            " am ",
                            " on ",
                    ...
                    has_arabic_location_pattern = (
                        any(
                            pattern in author_string.lower()
                            for pattern in [" al-", " al "]
                        )
                        and len(words) >= 2
                    )
                    ...
                    if (
                        first_word_capitalized
                        or has_location_connector
                        or has_arabic_location_pattern
                        or has_location_keyword
                    ):
                    ...
                    if (has_location_connector or has_arabic_location_pattern) and len(
                        words
                    ) >= 3:
                    ...
                    if (
                        ...
                        is_common_location_starter = (
                            words[0].lower() in common_location_starters
                        )
            ...
            try:
                ...
                if len(author_string.split()) == 1:
                    ...
                    if author_string[0].isupper() and len(author_string) >= 3:
                        ...
                        if (
                            author_string_lower in self.KNOWN_DEPARTMENTS
                            or "redaktion" in author_string_lower
                            or author_string_lower == "nzz"
                        ):
                ...
                if names:
                    ...
                    for loc in locations:
                        ...
                        if loc_lower in author_string_lower or any(
                            word in author_string_lower for word in loc_lower.split()
                        ):
                            ...
                            if len(loc.split()) > 1:
                                ...
                                is_name_match = any(
                                    name.lower() == loc_lower for name in names
                                )
                                if is_name_match:
                            ...
                            is_name_match = any(
                                name.lower() == loc_lower
                                or loc_lower in name.lower()
                                or name.lower() in loc_lower
                                for name in names
                            )
                    ...
                    for dept in departments:
                        ...
                        if dept_lower in author_string_lower or any(
                            word in author_string_lower for word in dept_lower.split()
                        ):
                            # CRITICAL: Don't allow names to be classified as departments
                            # If department matches a name, it's likely a false positive
                            is_name_match = any(
                                name.lower() == dept_lower
                                or dept_lower in name.lower()
                                or name.lower() in dept_lower
                                for name in names
                            )
                            if not is_name_match:
                    ...
                    for name in names:
                        ...
                        appears_in_original = (
                            any(word in author_string_lower for word in name_words)
                            or name_lower in author_string_lower
                        )

                        if not appears_in_original:
                            # Name doesn't appear in original - LLM made it up, skip it
                            logger.debug(
                                f"Filtering out name '{name}' - not found in original string '{author_string}'"
                            )
                            continue
                        ...
                        if len(name.split()) == 1:
                            # Skip if it's a known department
                            if (
                                name_lower in self.KNOWN_DEPARTMENTS
                                or "redaktion" in name_lower
                                or name_lower == "nzz"
                            ):
                    ...
                    if not names:
                        logger.debug(
                            f"LLM returned names but all were filtered out for '{author_string}', using heuristic parsing"
                        )
                        heuristic_results = self._parse_author_string_heuristic(
                            author_string
                        )
                    ...
                    for name in names:
                        ...
                        if len(name_parts) >= 2:
                            ...
                            appears_separately = any(
                                p == last_part_lower for p in original_parts
                            )
                            ...
                            is_likely_name = (
                                ...
                                and not self.is_location(
                                    last_part, context=author_string
                                )
                            )

                            if (
                                appears_separately
                                and not is_likely_name
                                and self.is_location(last_part, context=author_string)
                            ):
                                ...
                                location = (
                                    last_part  # Use this location for this author
                                )
                            else:
                        ...
                        author = self._parse_name_to_author(
                            clean_name, location, department, author_string
                        )
                        if author:
                            ...
                            if author.location:
                                ...
                                name_parts_lower = [
                                    p.lower() for p in author.normalized_name.split()
                                ]
                                loc_lower = author.location.lower()
                                if (
                                    loc_lower in name_parts_lower
                                    or author.location.lower()
                                    == author.normalized_name.lower()
                                ):
                ...
                if names and not parsed_authors:
                    logger.debug(
                        f"LLM found names but all were filtered out for '{author_string}', using heuristic parsing"
                    )
                    heuristic_results = self._parse_author_string_heuristic(
                        author_string
                    )
                ...
                if not names and not locations and not departments:
                    logger.debug(
                        f"LLM returned empty results for '{author_string}', using heuristic parsing"
                    )
                    heuristic_results = self._parse_author_string_heuristic(
                        author_string
                    )
                ...
                if not names and (locations or departments):
                    ...
                    if (
                        ...
                        return [author]
                    logger.debug(
                        f"LLM found only locations/departments for '{author_string}', using heuristic parsing"
                    )
            except Exception as e:
                logger.debug(
                    f"LLM parsing failed for '{author_string}': {str(e)}, falling back to heuristic parsing"
                )
    ...
    def _parse_author_string_heuristic(self, author_string: str) -> List[ParsedAuthor]:
        ...
        if len(words) == 1:
            ...
            if not is_name_with_prefix:
                ...
                has_location_connector = any(
                    connector in author_string.lower()
                    for connector in [" de ", " am ", " on ", " in ", " bei ", " an "]
                )
        ...
        if len(words) == 3:
            ...
            all_alpha_like = all(
                any(
                    c.isalpha() for c in w.replace("-", "").replace("'", "")
            ...
            if all_capitalized and all_alpha_like:
                ...
                is_location_keyword = any(
                    kw in middle_word.lower() for kw in location_keywords
                )
                ...
                is_middle_location = (
                    self.is_location(middle_word, context=author_string)
                ...
                if not is_location_keyword:
                    ...
                    author = self._parse_name_to_author(
                        author_string, None, None, author_string
                    )
                    if author:
                        return [author]
        ...
        if author_string.strip().startswith("Von ") or author_string.strip().startswith(
            "von "
        ):
            # Remove "Von" prefix and parse the rest
            name_without_von = (
                author_string.replace("Von ", "", 1).replace("von ", "", 1).strip()
            )
            if name_without_von:
                ...
                if len(name_parts) >= 2:
                    author = self._parse_name_to_author(
                        name_without_von, None, None, author_string
                    )
                    if author:
                        return [author]
        ...
        while i < len(parts):
            ...
            if "," in part and part.count(",") == 1:
                ...
                if name_part and location_part:
                    ...
                    if self.is_location(location_part, context=author_string):
                        ...
                        author = self._parse_name_to_author(
                            name_part, location_part, current_department, author_string
                        )
                        if author:
                            parsed_authors.append(author)
                        current_name_parts = []
            ...
            if (
                part_lower in self.KNOWN_DEPARTMENTS
                or "redaktion" in part_lower
                or part_lower == "nzz"
            ):
                is_dept = True
            else:
            ...
            if is_dept:
                if current_name_parts:
                    author = self._build_author(
                        current_name_parts, current_location, current_department
                    )
                    if author:
                        parsed_authors.append(author)
                    current_name_parts = []
            ...
            if self._looks_like_name(part) or (
                len(part.split()) >= 2
                and all(w[0].isupper() if w else False for w in part.split()[:2])
            ):
                current_name_parts.append(part)
                i += 1
                continue
            ...
            if current_name_parts:
                ...
                if i + 1 < len(parts) and (
                    self._looks_like_name(parts[i + 1].strip())
                    or len(parts[i + 1].strip().split()) >= 2
                ):
    ...
    def _parse_name_to_author(
        self,
        name: str,
        location: Optional[str],
        department: Optional[str],
        original_string: str,
    ) -> Optional[ParsedAuthor]:
        ...
        if len(name_parts) == 1:
            ...
            for i, part in enumerate(
                name_parts[1:-1], 1
            ):  # Check all except first and last
        ...
        if location:
            ...
            if location_lower in name_parts_lower:
                validated_location = (
                    None  # Location is part of the name, not a separate location
                )
            # Also check if location appears separately in original string
                ...
                if location_lower not in original_parts:
                    ...
                    if normalized_lower.endswith(
                        " " + location_lower
                    ) or normalized_lower.endswith(location_lower):
    ...
    def _looks_like_name(self, term: str) -> bool:
        ...
        if len(term.split()) == 1 and term[0].isupper() and len(term) >= 3:
            ...
            if (
                term_lower not in self.KNOWN_DEPARTMENTS
                and "redaktion" not in term_lower
                and term_lower != "nzz"
            ):
                # If it doesn't match common name patterns, it's likely a location
                if len(term) >= 5 or (
                    len(term) >= 3 and not self._is_common_name(term)
                ):
    ...
    def _build_author(
        self, name_parts: List[str], location: Optional[str], department: Optional[str]
    ) -> Optional[ParsedAuthor]:
...
if __name__ == "__main__":
    ...
    for test in test_cases:
        ...
        for author in authors:
            print(
                f"  -> {author.normalized_name} (Location: {author.location}, Dept: {author.department})"
            )
