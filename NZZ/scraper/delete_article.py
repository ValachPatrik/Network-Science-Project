...
def delete_article_by_id(
    article_id: str, show_info: bool = True, confirm: bool = True
) -> bool:
    ...
    try:
        ...
        if confirm:
            response = (
                input(
                    f"\nAre you sure you want to delete article '{article_id}'? (yes/no): "
                )
                .strip()
                .lower()
            )
...
def delete_articles_by_ids(
    article_ids: list, show_info: bool = True, confirm: bool = True
) -> int:
    ...
    try:
        ...
        if confirm:
            response = (
                input(
                    f"\nAre you sure you want to delete {len(articles)} article(s)? (yes/no): "
                )
                .strip()
                .lower()
            )
        ...
        if not_found:
            print(
                f"[!] {len(not_found)} article(s) were not found and could not be deleted."
            )
...
Examples:
  ...
  python delete_article.py 1671603 --no-info
    ...
    parser.add_argument(
        "article_ids", nargs="*", metavar="ID", help="Article ID(s) to delete"
    )
    parser.add_argument(
        "--file",
        "-f",
    ...
    if not article_ids:
        parser.print_help()
        print(
            "\n[X] No article IDs provided. Use --help for usage information.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        if len(article_ids) == 1:
            ...
            success = delete_article_by_id(
                article_ids[0], show_info=show_info, confirm=confirm
            )
            sys.exit(0 if success else 1)
        else:
            # Multiple articles deletion
            deleted_count = delete_articles_by_ids(
                article_ids, show_info=show_info, confirm=confirm
            )
