...
class AuthorArticleDataSyncer:
    ...
    def _clean_text(self, text):
        ...
        cleaned = re.sub(
            r"&#x([0-9a-fA-F]+);?", lambda m: chr(int(m.group(1), 16)), cleaned
        )
    ...
    def build_name_mapping(self, authors_df):
        ...
        logger.info(
            f"Built name mapping for {len(name_to_authors):,} unique author names"
        )
    ...
    def sync_data(self, articles_df, authors_df, name_to_authors):
        ...
        for _, article in articles_df.iterrows():
            ...
            for author_name in author_names:
                ...
                if normalized_name and normalized_name in name_to_authors:
                    ...
                    for author_record in name_to_authors[normalized_name]:
                        ...
                        authors_updates[author_id]["department"] = (
                            self._merge_list_values(existing_dept, article_dept)
                        )

                        # Merge location (as list)
                        existing_loc = authors_updates[author_id]["location"]
                        authors_updates[author_id]["location"] = (
                            self._merge_list_values(existing_loc, article_loc)
                        )
            ...
            for author_name in author_names:
                ...
                if normalized_name and normalized_name in name_to_authors:
                    for author_record in name_to_authors[normalized_name]:
                        if (
                            pd.notna(author_record["department"])
                            and author_record["department"]
                        ):
                            article_depts.append(author_record["department"])
                        if (
                            pd.notna(author_record["location"])
                            and author_record["location"]
                        ):
            ...
            article_dept_parsed = (
                self._parse_list_value(article_dept) if pd.notna(article_dept) else []
            )
            merged_dept_parsed = (
                self._parse_list_value(merged_dept) if merged_dept else []
            )
            article_loc_parsed = (
                self._parse_list_value(article_loc) if pd.notna(article_loc) else []
            )
            merged_loc_parsed = self._parse_list_value(merged_loc) if merged_loc else []

            # Check if values changed (compare normalized lists)
            dept_changed = sorted([x.lower() for x in article_dept_parsed]) != sorted(
                [x.lower() for x in merged_dept_parsed]
            )
            loc_changed = sorted([x.lower() for x in article_loc_parsed]) != sorted(
                [x.lower() for x in merged_loc_parsed]
            )
        ...
        for _, author in authors_df.iterrows():
            ...
            # Compare by parsing and normalizing JSON strings
            existing_dept_parsed = (
                self._parse_list_value(existing_dept) if pd.notna(existing_dept) else []
            )
            final_dept_parsed = self._parse_list_value(final_dept) if final_dept else []
            existing_loc_parsed = (
                self._parse_list_value(existing_loc) if pd.notna(existing_loc) else []
            )
            final_loc_parsed = self._parse_list_value(final_loc) if final_loc else []

            # Check if values changed (compare normalized lists)
            dept_changed = sorted([x.lower() for x in existing_dept_parsed]) != sorted(
                [x.lower() for x in final_dept_parsed]
            )
            loc_changed = sorted([x.lower() for x in existing_loc_parsed]) != sorted(
                [x.lower() for x in final_loc_parsed]
            )
            ...
            with self.engine.begin() as conn:  # Use transaction
                ...
                for article_id, data in articles_updates.items():
                    ...
                    if articles_updated % batch_size == 0:
                        logger.info(
                            f"Updated {articles_updated:,} / {len(articles_updates):,} articles"
                        )
                ...
                for author_id, data in authors_updates.items():
                    ...
                    if authors_updated % batch_size == 0:
                        logger.info(
                            f"Updated {authors_updated:,} / {len(authors_updates):,} authors"
                        )
    ...
    def run(self, dry_run=False):
        ...
        try:
            ...
            # Sync data
            articles_updates, authors_updates, articles_original, authors_original = (
                self.sync_data(articles_df, authors_df, name_to_authors)
            )
            ...
            if dry_run:
                ...
                if len(articles_updates) == 0:
                    ...
                    for i, (article_id, data) in enumerate(
                        list(articles_updates.items())[:20]
                    ):
                    ...
                    if len(articles_updates) > 20:
                        logger.info(
                            f"  ... and {len(articles_updates) - 20} more articles"
                        )
                ...
                if len(authors_updates) == 0:
                    ...
                    for i, (author_id, data) in enumerate(
                        list(authors_updates.items())[:20]
                    ):
                        original = authors_original[author_id]
                        logger.info(
                            f"Author ID: {author_id} (Name: {original['name']})"
                        )
                        logger.info("  Department:")
                        logger.info(f"    BEFORE: {original['department']}")
                        logger.info(f"    AFTER:  {data['department']}")
                    ...
                    if len(authors_updates) > 20:
                        logger.info(
                            f"  ... and {len(authors_updates) - 20} more authors"
                        )
