...
class LLMLocationClassifier:
    ...
    def __init__(self, model_name: str = "gemma3:270m", use_llm: bool = True, ollama_base_url: Optional[str] = None):
        ...
        try:
            ...
            try:
                ...
                if self.model_name not in model_names:
                    ...
                    logger.warning(f"Please install it with: ollama pull {self.model_name}")
                    logger.warning("LLM classification will be disabled.")
                    self.use_llm = False
                    return
                logger.info(f"LLM classifier initialized with Ollama model: {self.model_name} (Base URL: {self.ollama_base_url})")
            except Exception as e:
                logger.warning(f"Could not verify Ollama model '{self.model_name}': {str(e)}")
    ...
    def is_location(self, term: str, context: Optional[str] = None, min_confidence: float = 0.6) -> bool:
    ...
    def is_name(self, term: str, context: Optional[str] = None, min_confidence: float = 0.6) -> bool:
    ...
    def is_department(self, term: str, context: Optional[str] = None, min_confidence: float = 0.6) -> bool:
